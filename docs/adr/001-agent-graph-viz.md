...
- **Nodes:**
  ...
  - File: label-only (no marker rectangle as of #569), label = basename (last path component). On collision, fall back to last 2 path components separated by `/`.
...
> **Addendum (#569, 2026-05-01):** The file branch previously drew a 0.04×0.04 marker rectangle at `(p.x, p.y)` and anchored the label at `(p.x, p.y - 0.08)`. The 0.08-unit drop equates to roughly one terminal row at 80×24, leaving a visible vertical gap between the incoming edge endpoint and the label's top cell — the label looked disconnected from its edge (issue #569). The fix drops the marker entirely and anchors the label at `(p.x, p.y)`, letting the label's first glyph serve as the visual node. The edge therefore terminates inside the label's leading character; the gap is impossible by construction. The marker carried no semantic information (no status, no animation, no modifier), so removing it loses nothing. Agent nodes are unchanged: they retain their sprite or rectangle because they convey role/status that a single glyph cannot. If a future feature needs a per-file status indicator, prepend an icon character to the label (e.g. `⚠ main.rs`) rather than re-introducing a separate marker. No change to the layout algorithm, data model, or trait boundary.

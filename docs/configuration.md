...
| `role_overrides` | table | `{}` | Rich-form bindings: see `[teams.<id>.role_overrides.<role>]`. Fully editable in the TUI Settings → Teams tab as of v0.29.x (#901) — the `role_overrides` row opens a nested editor with one sub-tab per role and the 5 sub-fields (`agent`, `mode`, `model_override`, `prompt_addendum`, `fallback_agent`); `[a]` adds a role, `[d]` removes the active role. Empty `role_overrides` does not emit a bare table header on save. |

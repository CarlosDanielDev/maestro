...
pub fn build_validator_map() -> std::collections::HashMap<FieldKey, ValidatorFn> {
    ...
    // (1, 13): context_overflow.overflow_threshold_pct after 2026-05-23
    // inserted `session_history_cap` between max_prompt_history and
    // guardrail_prompt. Layout:
    // [0]max_concurrent, [1]stall_timeout_secs, [2]default_mode,
    // [3]bypass_review_corrections, [4]allowed_tools, [5]max_retries,
    // [6]retry_cooldown_secs, [7]max_prompt_history,
    // [8]session_history_cap, [9]guardrail_prompt,
    // [10..12]hollow_retry.*, [13]context_overflow.overflow_threshold_pct.
    m.insert((1, 13), validate_overflow_threshold_pct as ValidatorFn);
    // Budget tab (2)
...
mod tests {
    ...
    fn validator_keys_match_expected_field_labels() {
        ...
        let expectations: &[((usize, usize), &str)] = &[
            ...
            // Shifted from idx 12 → 13 after 2026-05-23 inserted
            // session_history_cap between max_prompt_history and
            // guardrail_prompt.
            ((1, 13), "context_overflow.overflow_threshold_pct"),
...
"#;
    ...
    fn validator_map_has_all_entries() {
        ...
        assert!(map.contains_key(&(1, 13))); // context_overflow.overflow_threshold_pct (post session_history_cap shift)

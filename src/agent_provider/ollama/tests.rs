...
async fn emits_context_update_from_usage_block_when_num_ctx_set() {
    ...
    while let Some(event) = rx.recv().await {
        if let AgentProviderEvent::Stream(StreamEvent::ContextUpdate { context_pct: pct }) = event {
...
async fn does_not_emit_context_update_when_num_ctx_unset() {
    ...
    .await;
    let provider = OllamaProvider::new("ollama", base_url, "llama3.2", 5, None).expect("provider");
    let (tx, mut rx) = mpsc::unbounded_channel();

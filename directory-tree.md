...
│   │   │   ├── model.rs                   # GraphNode, GraphEdge, NodeId, NodeKind, Positioned data types  [Issue #526]
...
│   │   │   └── render.rs                  # ratatui Canvas renderer; draws edges as Lines, nodes as Rectangles + labels; single-agent fallback card; min-size guard (80×24)  [Issue #526]
...
│   │   ├── snapshot_tests/                # TUI snapshot tests using insta (45 tests, 10 views)  [Issue #16, #490, #526]
│   │   │   ├── mod.rs                     # Module declarations for snapshot test submodules; mod agent_graph wired  [Issue #526]
...
│   │   │   └── snapshots/                 # Committed insta snapshot files (.snap files); includes caveman_row renders (default, error, explicit_false, explicit_true, focused_explicit_true); copy_keybinding_hint_enabled and copy_keybinding_hint_disabled; agent_graph renders at 80x24, 100x30, 120x40, and single_agent_card  [Issue #490, #482, #526]
...
| `src/tui/agent_graph/model.rs` | `GraphNode`, `GraphEdge`, `NodeId`, `NodeKind`, `Positioned` data types (Issue #526) |
...
| `src/tui/snapshot_tests/` | TUI snapshot test suite; 45 tests across 10 views using `insta`; run with `cargo test tui::snapshot_tests`; update with `INSTA_UPDATE=always cargo test` or `cargo insta review` (Issues #16, #490, #526) |
...
| `src/tui/snapshot_tests/snapshots/` | Committed `.snap` files — insta ground-truth for TUI rendering regressions; includes 4 agent_graph baselines (`80x24`, `100x30`, `120x40`, `single_agent_card`) added in Issue #526 |

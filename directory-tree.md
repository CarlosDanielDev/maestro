...
| `src/agent_provider/types.rs` | `AgentProvider` trait + `AgentProviderId`, `AgentProviderKind`, `AgentOutputFormat`, `ParserBinding`; `template_rules()` default method returns `null_rules()`; providers with dedicated rules (e.g. Claude, Codex, HTTP-generic) override this method (Issues #701, #703, #704, #705) |
...
| `src/agent_provider/ollama/mod.rs` | `OllamaProvider` impl; `template_rules()` override returns `http_generic_rules()` (Issue #705) |
| `src/agent_provider/minimax/mod.rs` | `MiniMaxProvider` impl; `template_rules()` override returns `http_generic_rules()` (Issue #705) |

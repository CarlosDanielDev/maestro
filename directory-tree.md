...
│   │   │   ├── animation.rs               # Stateless animation helpers; SessionRenderInfo (Clone) snapshot struct carrying tick + session_state; edge_color() derives LightCyan/Cyan/DarkGray pulse on tool-use prefix match (tick % 6 < 5); node_animation_style() returns per-tick BOLD/BOLD|REVERSED flash for Completed/Errored (parity-modulated via transition_flash_remaining) and braille spinner glyph for Running via graph_node_frame(); all items pub(super)  [Issue #529]
│   │   │   ├── render.rs                  # ratatui Canvas renderer; draws edges as Lines, nodes as Rectangles + labels; single-agent fallback card; min-size guard (80×24); signature extended with use_nerd_font: bool, tick: usize, sessions: &[&Session]; Running node labels carry per-frame braille/ASCII spinner glyph from graph_node_frame(); edge pulse on current_activity tool-use prefix (LightCyan/Cyan/DarkGray cycle, tick % 6 < 5); Completed/Errored status-transition flash consumes Session::transition_flash_remaining with parity-modulated BOLD/BOLD|REVERSED; tests extracted to render_tests.rs sibling  [Issue #526, #527, #529]
...
│   │   │   ├── auto_pr.rs                 # Auto-PR pipeline (#514): pr_url() helper, sanitize_log() for error messages, App::run_auto_pr() — decides whether to create a PR, detects existing open PRs via list_prs_for_branch(), logs the PR URL on success, surfaces every gate-skip and error path with explicit activity-log entries, appends manual-recovery hint on general error  [Issue #514]
│   │   │   ├── auto_pr_tests.rs           # Behavior tests for auto_pr.rs (8 tests covering AC1, AC4, AC6, AC7, G.1–G.3: PR URL logged, existing-PR detection, recovery hint, idempotency guard, silent-gate logging)  [Issue #514]
│   │   │   ├── issue_completion.rs        # on_issue_session_completed(): thin entry point (~150 LOC) that delegates auto-PR work to auto_pr::run_auto_pr(); skips PR creation for CI-fix sessions  [Issue #514]
...
│   │   │   ├── agent_graph.rs             # 10 snapshot tests for agent graph: renders_at_80x24, renders_at_100x30, renders_at_120x40, single_agent_card (original 4); agent_graph_running_node_braille_spinner_at_tick_0, agent_graph_running_node_braille_spinner_at_tick_5, agent_graph_completed_flash_tick4_bold_reversed, agent_graph_edge_pulse_tooluse_tick0, agent_graph_edge_pulse_tooluse_tick5, agent_graph_ascii_fallback_spinner_running_node (6 new); render helper signature updated to accept tick + nerd-font flag  [Issue #526, #529]
...
| `src/tui/agent_graph/render.rs` | ratatui `Canvas` renderer; draws edges as `Line` shapes, nodes as `Rectangle` + label; single-agent fallback card; 80×24 min-size guard (Issue #526) |
...
| `src/tui/snapshot_tests/` | TUI snapshot test suite; 48 tests across 11 views using `insta`; run with `cargo test tui::snapshot_tests`; update with `INSTA_UPDATE=always cargo test` or `cargo insta review` (Issues #16, #490, #526, #527) |
...
| `src/tui/snapshot_tests/snapshots/` | Committed `.snap` files — insta ground-truth for TUI rendering regressions; includes 4 agent_graph baselines (`80x24`, `100x30`, `120x40`, `single_agent_card`) added in Issue #526; 2 agent_graph_dispatcher baselines (`toggle_on_renders_graph`, `toggle_off_renders_panels`) added in Issue #527 |
...
| `Cargo.toml` | Package manifest; `[features] spike = []` block and `[[example]] agent_graph_spike` entry removed; `experimental-sanitizer` feature retained (Issue #526) |

...
│   ├── insight/                           # Static maintenance-map extractor — walks the repo and emits docs/insight/scan.json  [Issue #1002]
│   │   ├── mod.rs                         # Module facade; `scan(root) -> Scan` orchestrator + `run_cli(root) -> Result<()>` CLI entry point; populates repo_stats and modules (P1); collects features via `collect_features` (4 surfaces: cli, tui_mode, slash_command, subagent) and maps modules + coverage (P2); `collect_design_system(root)` wired into scan() (P3)  [Issue #1002, #1003, #1004]
│   │   ├── schema.rs                      # Versioned serde structs: `Scan`, `Feature`, `Module`, `RepoStats`, `Release`, `DesignSystem`, `Architecture`, `Coverage`, `SurfaceType`, `NamedValue`, `Mascot`, `Edge`; single source of truth for docs/insight/scan.json shape; bump `Scan::schema_version` on any breaking change  [Issue #1002]
│   │   ├── modules.rs                     # `analyze_source(path, src) -> Module` — syn-based extractor; collects public API names, doc comments, and use-edge imports from Rust source files; `extract_doc` raised to `pub(crate)` for reuse by coverage  [Issue #1002, #1003]
│   │   ├── features.rs                    # Feature surface extraction (P2): `surfaces_from_enum` parses CLI `Commands` and TUI `TuiMode` variants via syn; `surfaces_from_md_dir` collects slash-command templates and subagent .md files; `slug_id` and `kebab` helpers  [Issue #1003]
│   │   ├── coverage.rs                    # Coverage computation (P2): `map_modules` links features to modules via heuristic name matching; `compute_coverage` produces `surfaces_total`, `surfaces_documented`, and `modules_orphaned` counts  [Issue #1003]
│   │   ├── design_system/                 # Design-system extractor (P3): reads Theme::dark() fields and icon_pair() match arms at compile time  [Issue #1004]
│   │   │   ├── mod.rs                     # `collect_design_system(root) -> DesignSystem`; extracts palette (40 entries from Theme::dark() fields), icons (46 entries from icon_pair() match arms), and mascot (Clawd, frame_count 6); styles and layout_conventions deferred to later phase  [Issue #1004]
│   │   │   └── tests.rs                   # Unit tests for design_system extractor  [Issue #1004]
│   │   └── repo_stats.rs                  # Repository statistics collectors: `parse_releases`, `loc_by_lang`, `git_stats`, `collect(root) -> RepoStats`  [Issue #1002]
│   ├── main.rs                            # CLI entry point (clap); Run, Queue, Add, Status, Cost, Init, Doctor; --skip-doctor flag on Run subcommand bypasses preflight; cmd_run() runs validate_preflight() before session launch and uses PromptBuilder::build_issue_prompt() for issue sessions; setup_app_from_config() shared helper wires budget, model router, notifications, plugins, and permission_mode/allowed_tools from config; propagates once_mode from parsed CLI flag into App; forces max_concurrent=1 when --continuous is set; cmd_dashboard() performs orphan worktree cleanup, log cleanup, fetches username from doctor report, delegates App construction to setup_app_from_config(), and queues FetchSuggestionData on startup; invokes config::run_startup_migration(Path::new("maestro.toml")) after Cli::parse() to backfill default fields in existing configs; declares #[cfg(test)] mod integration_tests; declares mod updater; declares mod flags; propagates startup gh auth check result into App.gh_auth_ok; declares mod sanitize; constructs FeatureFlags from --enable-flag / --disable-flag CLI args merged with [flags] config; dispatches Commands::SyncTemplates to cmd_sync_templates()  [Issue #15, #29, #49, #34, #36, #35, #52, #83, #85, #118, #141, #142, #143, #158, #706, #710]
│   ├── cli.rs                             # CLI definition extracted from main.rs; Cli struct and Commands enum (clap derive); --once flag on Run subcommand (exits after all sessions complete, for CI/scripting); --continuous / -C flag on Run subcommand (auto-advance through issues, pause on failure); --enable-flag / --disable-flag repeatable args on Run subcommand for runtime feature flag overrides; --bypass-review global flag (session-only, skips review council); generate_completions() and cmd_completions() for shell tab-completion output; cmd_mangen() for roff man page generation; Completions and Mangen subcommands; Commands::Team { action } variant added for team subcommands; mod cli_team declared; Commands::SyncTemplates { provider, check, dry_run } variant added; 5 parse tests for sync-templates surface; 14 parse tests for team subcommand surface  [Issue #18, #83, #85, #143, #328, #665, #706]
...
│   │   ├── mod.rs                         # Module re-exports; `mod doctor` widened to `pub mod doctor` to allow integration-test access; `pub mod team` added; `pub(crate) mod agent_provider_map` added  [Issue #663, #665, #1000]
│   │   ├── agent_provider_map.rs          # `enabled_agent_ids()` + `build_agent_provider_map()` relocated from `src/tui/app/mod.rs`; builds the per-agent provider map for headless team-launch without a TUI dependency  [Issue #1000]
│   │   ├── clean.rs                       # cmd_clean(): prune orphaned worktrees and stale log files
│   │   ├── dashboard.rs                   # cmd_dashboard(): launch the TUI dashboard; wires MinimaxQuota into App best-effort via App::with_minimax_quota  [Issue #769]
...
│   │   ├── run.rs                         # cmd_run(): validate preflight then launch a session; minimax_quota_path() raised to pub(crate) for reuse  [Issue #769]
...
│   │   ├── status.rs                      # cmd_status(): print current session and queue state; pub fn format_cost_report(state: &MaestroState) -> String — pure formatter with per-provider BTreeMap rollup by agent_id; cmd_cost delegates to it  [Issue #769]
│   │   ├── sync_templates/                # `maestro sync-templates` subcommand: renders canonical .maestro/templates/commands/*.md per provider and writes results to provider target dirs; records SHA-256 checksums in .maestro/templates.lock  [Issue #706]
│   │   │   ├── mod.rs                     # CLI entry point: cmd_sync_templates(SyncTemplatesArgs); dispatches to SyncRunner with real filesystem
│   │   │   ├── registry.rs                # PROVIDERS: &[ProviderEntry] (6 providers: claude, codex, opencode, qwen, ollama, minimax), COMMANDS: &[&str]; entries_for(filter) applies --provider filter
│   │   │   ├── banner.rs                  # with_banner(rendered, command) -> String; prepends AUTO-GENERATED banner comment to rendered output
│   │   │   ├── lockfile.rs                # Lockfile, FileEntry, sha256_hex(); reads/writes .maestro/templates.lock (TOML, deterministic, sorted keys)
│   │   │   ├── diff.rs                    # line_diff(expected, actual) -> String; used by --check mode to report drift to stderr
│   │   │   ├── runner.rs                  # SyncRunner, SyncFs trait, RealFs (rejects symlinks), SyncOutcome, SyncTemplatesError; --check re-renders + diffs; --dry-run prints planned writes; OpenCode uses NullRules (skipped with tracing::warn!)
...
│   │   └── team.rs                        # cmd_team(): dispatch arm for all `maestro team` subcommands; SchedulerRunner trait + ProductionSchedulerRunner impl for headless `team launch --yes` execution; routes TeamAction variants to list/new/explain/manage/launch handlers; `ProductionSchedulerRunner::run_issue` now injects a factory with the populated `agent_providers` map via `agent_provider_map::build_agent_provider_map()`  [Issue #665, #1000]
│   ├── config/                            # maestro.toml parsing — split from the former config.rs monolith  [Issue #29, #40, #41, #43, #38, #143, #275, #437, #473, #525, #710, #712, #713, #718]
│   │   ├── mod.rs                         # Module facade; re-exports all Config types (ModelsConfig, GatesConfig, ReviewConfig, ViewsConfig, etc.); re-exports run_startup_migration() and MigrationResult from migrate.rs; find_and_load_with_path() / find_and_load_in_with_path(); legacy find_and_load() / find_and_load_in() shims; Config::save dispatches via fs::metadata (NotFound/empty → save_fresh; otherwise → save_into_existing); Config::save_into_str(&self, original_text) comment-preserving overlay merge via overlay.rs; Config::save_into_existing(path) atomic-write wrapper; Config::save_fresh(path) fast path for new installs; pub mod schema; mod toml_edit_helpers  [Issue #712, #713, #718]
...
│   │   ├── migrate.rs                     # Startup config migration for v0.25.1+ defaults: run_startup_migration(path) idempotently adds agent_graph_enabled = true to [views] when absent via ensure_field (toml_edit_helpers); comment-, blank-line-, and key-order-preserving rewrite via toml_edit::DocumentMut; security-hardened: rejects symlinks and non-regular files via symlink_metadata(), refuses files > 1 MiB; uses tempfile::NamedTempFile (O_EXCL) for atomic write to close TOCTOU window; returns MigrationOutcome enum (NoChange / Migrated); loads byte-identity test group via #[path] include  [Issue #710, #718]
...
│   │   ├── toml_edit_helpers.rs           # Shared pub(crate) helper for single-key comment-preserving TOML edits: ensure_field(doc, table_path, key, value) inserts the key only when absent, returns EnsureOutcome enum (Inserted / AlreadyPresent); ~80 LOC prod + inline unit tests  [Issue #718]
│   │   ├── budget.rs                      # Budget config types
│   │   ├── experimental.rs                # ExperimentalConfig (azure_devops flag; retired in v0.24.0)
│   │   ├── flags.rs                       # FlagsConfig (flattened HashMap<String, bool>) loaded from [flags] table
│   │   ├── gates.rs                       # GatesConfig, CompletionGatesConfig, CompletionGateEntry, CiAutoFixConfig
│   │   ├── github.rs                      # GitHub provider config types
...
│   │   │   ├── roundtrip_role_overrides.rs  # TUI-driven round-trip tests for role_overrides: empty map omits bare header, populated map survives save/reload, passthrough preserves existing keys through EntryState; add + edit tests added in #901  [Issue #872, #901]
...
│   │   ├── schema.rs                      # FieldSchema, FieldKind, TableSchema, DefaultValue, Validator types; Presentation enum (Subtabs | Rows) renderer hint; FieldKind::Map { entry_fields } and FieldKind::VecOfStruct { entry_fields } dynamic-cardinality variants; DefaultValue::Empty marker for dynamic sections; FieldSchema.presentation: Option<Presentation> field; FieldSchema::resolved_presentation() const method; pub(crate) validate_url_or_empty and validate_non_empty validators; pub(crate) const PROJECT_TABLE: TableSchema — canonical schema for the Project settings tab; inline unit tests moved to schema_tests.rs sidecar  [Issue #713, #715, #789]
│   │   ├── schema_tests.rs                # Sidecar test module for schema.rs loaded via `#[path]`; split from inline mod tests to keep schema.rs under the 400-LOC cap; covers: all static dotted paths resolve on default Config, enum defaults within allowed variants, help-text style, DefaultValue/FieldKind compatibility (including Empty + Map/VecOfStruct), no duplicate dotted paths, validator purity, Presentation resolution for Map/VecOfStruct/static variants, Debug stability, and const-constructibility of dynamic FieldKind variants  [Issue #789]
...
│   │   │   ├── dynamic.rs                 # `FieldKind::FlattenedMap` schema variant — entries live at the parent table level directly (used for `agents.<id>` and `modes.<id>`); `AGENTS_TABLE` and `MODES_TABLE` const schemas; `truncated_titles` helper for sub-tab strip overflow when 12+ entries; `ROLE_OVERRIDE_FIELDS` const (5 fields: agent, mode, model_override, prompt_addendum, fallback_agent) added as the 5th slot in `TEAMS_ENTRY_FIELDS` via `FieldKind::Map { entry_fields: ROLE_OVERRIDE_FIELDS }` — fully editable as of #901  [Issue #792, #872, #901]
...
│   │   │       └── extras.rs              # Extended registry entries (experimental, flags)
...
│   │   ├── sanitize.rs                    # Value-clamp primitives shared between budget projector and TUI rollup; deduped from inline clamping logic  [Issue #854]
│   │   └── test_support.rs               # `#[cfg(test)]` shared `FakeProviderQuotaSnapshots` fake — eliminates per-module duplicate fakes  [Issue #854]
│   ├── tui/budget_prespawn.rs             # Pre-spawn budget gate modal widget: renders [y]es/[n]o/[s]kip confirmation overlay before session spawn when projected spend exceeds threshold  [Issue #850]
│   ├── tui/budget_banner.rs               # 80% global budget banner widget: top-chrome warning strip rendered when aggregate spend crosses 80% of the configured total budget  [Issue #850]
...
│   ├── git_tests.rs                       # Unit tests for git.rs extracted to sibling file (attached via #[cfg(test)] #[path]); covers commit_and_push flag-injection refusal, backup_wip sentinel subject line, amend_clean_and_push happy path, head_is_wip_backup predicate, and -- separator presence in push argv  [Issue #562]
...
│   │   ├── sanitize.rs                    # sanitize_log(): strips ASCII control chars (replace-with-space) from external `git`/`gh` stderr before it reaches the activity log; defeats terminal-escape / label-spoofing (#514 LOW-1). Hoisted out of auto_pr.rs in #562 so completion_git.rs can re-use without a cross-sibling pub(super) import  [Issue #514, #562]
...
│   │   ├── mod.rs                         # Flag enum (7 variants: ContinuousMode, AutoFork, CiAutoFix, ReviewCouncil, ModelRouting, ContextOverflow, TurboQuant); FlagSource enum (Default, Config, Cli); serde serialization; default_enabled(), description(), name(), all() helpers  [Issue #714, #716]
...
│   │   ├── rendered_cache.rs              # `RenderedTemplateStore` trait + `DiskRenderedTemplateStore` impl + `FakeRenderedStore` test double; disk impl reads pre-rendered template bodies from the provider's cache dir; injected into `SessionPool::try_promote` for HTTP-generic providers (Issue #707)
│   │   ├── provider_rules/
│   │   │   ├── mod.rs                     # `TemplateProviderRules` trait (per-provider placeholder validation + unsupported-feature guard); `NullRules` fail-closed stub; `null_rules()` free fn returning `&'static NullRules`; re-exports `ClaudeRules`, `codex_rules()`, and `http_generic_rules()`; `read_sandboxed` + `read_skill_body` shared helpers (consolidates three near-identical per-file readers); Unix-gated `symlink_escape_is_rejected_by_starts_with_check` test  [Issue #701, #703, #704, #705, #727]
│   │   │   ├── claude.rs                  # `ClaudeRules` — concrete `TemplateProviderRules` impl for Claude Code; `include()` delegates to `read_sandboxed`; 8 unit tests  [Issue #703, #727]
│   │   │   ├── codex.rs                   # `CodexRules` — concrete `TemplateProviderRules` impl for Codex; `target_dir()=None`; `invoke_subagent` inlines as `## Sub-task`; `hook_gate` emits provider-neutral bash path; `include()` delegates to `read_sandboxed`; `skill_link` delegates to `read_skill_body`; `subagent_list` delegates to `subagent_list::load_subagent_list_markdown()`  [Issue #704, #727, #728]
│   │   │   ├── http_generic.rs            # `HttpGenericRules` — shared `TemplateProviderRules` for HTTP-transport providers (Ollama, MiniMax) and Qwen; `target_dir()=None`; `hook_gate` emits orchestrator instruction text (not a shell command); `include()` delegates to `read_sandboxed`; `skill_link` delegates to `read_skill_body`; `subagent_list` delegates to `subagent_list::load_subagent_list_markdown()`  [Issue #705, #727, #728]
│   │   │   └── subagent_list.rs           # Shared `{{SUBAGENT_LIST}}` renderer; `render_subagent_list_default(&[ManifestSubagent])` — pure fn, table header + rows, pipeline order, no trailing newline; `load_subagent_list_markdown()` — parses manifest and delegates; 7 unit tests  [Issue #728]
│   │   └── renderer/
...
│   │   ├── team_role_overrides.rs         # Soft cross-table validator: `RoleOverrideField` enum (Agent/Mode/FallbackAgent), `RoleOverrideWarning` struct, `validate_role_overrides(&HashMap<String, TeamConfig>, agents, modes) -> Vec<RoleOverrideWarning>`; empty/whitespace = inherit (no warning); `RoleOverrideWarning::message(&BTreeSet<String>)` enumerates configured ids (cap 6 + overflow) so inline warnings list valid options instead of just `unknown <kind>`; wired into settings/mod.rs::save_config; Save proceeds on soft warnings  [Issue #908, #909, #912]
│   │   ├── team_role_overrides_tests.rs   # Sibling test file split from team_role_overrides.rs to keep production file ≤ 400 LOC; covers validator branches + message-enrichment cases  [Issue #912]
...
│   │   ├── pool.rs                        # Session pool: max_concurrent, queue, auto-promote; branch tracking; guardrail_prompt field; set_guardrail_prompt(); merged into system prompt in try_promote(); find_by_issue_mut(); decrements flash_counter on each session per render tick and emits STATUS activity log entries on state transitions; `interactions: Vec<InteractionSession>` field; `find_active_interaction_by_issue()`; `create_interaction_session` (worktree), `clone_active_interaction`, `upsert_interaction` (with terminated-resurrection guard)  [Phase 3, Issue #40, #43, #202, #734, #738]
...
│   │   ├── interaction.rs                 # `InteractionSession`, `TurnRole`, `CloseReason`, `TurnRecord`, `TurnState` types; `new()` / `is_active()` constructors; `queued_terminator` field; `signal_terminator()` / `fire_terminator()` state-machine methods; `settled_banner()` helper for the settled-from status text; `interactive_banner()` on `Session`; `InteractionState` removed in #950  [Issue #734, #739, #950]
...
│   │   └── interaction_turn.rs            # Per-turn agent loop on the `AgentProvider` seam (#751 rewired #737): `InteractionSession::send_turn(provider)` delegates to `provider.run` with `resume_session_id`; `build_turn_request`; `TurnEvent` / `TurnError`; session_id binds from `AgentRunResult`; `interaction.turn` tracing span (#742); degraded mode  [Issues #737, #751, #742]
│   ├── state/
...
│   │   │   ├── label_placement.rs         # LabelPlacement: computes the largest angular gap between all outbound edges from an agent node and places the #NNN chip at the midpoint of that gap (north fallback when no edges); anchor shifted by (1 − cos θ) / 2 × label_width to avoid the edge bundle on east/west placements; place_file_label() decides per-side anchor (right-half grows rightward, left-half grows leftward, center band centered) using FILE_LABEL_DEAD_BAND and FILE_LABEL_BORDER_MARGIN_CELLS constants; truncate_with_ellipsis() trims labels that overshoot the outward span  [Issue #567, #568]
│   │   │   ├── render.rs                  # ratatui Canvas renderer; draws edges as Lines, agent nodes as Rectangles + labels, file nodes as label-only (no marker rectangle, label anchored at p.y per #569); sprite/abbrev rendering via personalities module; status_modifier() helper; single-agent fallback card; 80×24 min-size guard; signature extended with use_nerd_font: bool, tick: usize, sessions: &[&Session]; Running node labels carry braille/ASCII spinner glyph; edge pulse on current_activity; Completed/Errored flash via parity-modulated BOLD/BOLD|REVERSED; tests extracted to render_tests.rs sibling; agent chip placement delegated to label_placement::LabelPlacement; file-arm labels placed via label_placement::place_file_label; draw_sprite_on_canvas takes inner_cols and inner_rows to derive viewport-relative row_step and x_offset; LABEL_RADIUS_SPRITE removed, replaced with SPRITE_LABEL_BUFFER_CELLS  [Issue #526, #527, #529, #539, #567, #568, #569, #576]
...
│   │   │   ├── render_sprite_tests.rs     # Viewport-aware sprite-geometry tests (Issue #576): 7 behavioral assertions for sprite contiguity, height budget, and horizontal centering across 80×24, 120×40, 200×60. Helpers: render_buffer_sized, agent_col, sprite_cells. Linked via #[cfg(test)] #[path = "render_sprite_tests.rs"] mod sprite_tests in render.rs  [Issue #576]
│   │   │   └── render_label_placement_tests.rs  # Regression tests for label_placement: agent_label_does_not_overlap_south_edge and agent_label_does_not_overlap_south_edge_nerd_font pin corrected placement via insta snapshots; linked via #[cfg(test)] #[path = "render_label_placement_tests.rs"] mod label_placement_tests in render.rs  [Issue #567]
│   │   ├── app/                           # App state module (split across multiple files)
│   │   │   ├── mod.rs                     # App struct; nav_stack: NavigationStack field (replaces confirm_exit_return_mode); navigate_to(), navigate_back(), navigate_back_or_dashboard(), navigate_to_root() navigation methods; gh_auth_ok: bool; AUTH_RECOVERY_HINT constant; cap-coincidence note rehydrated on App::new(); theme: Theme; pending_prs: Vec<PendingPr>; config_path: Option<PathBuf> field carries the resolved maestro.toml path for settings save; set_config_path() setter; process_pending_pr_retries(); trigger_manual_pr_retry(); last_pr_created_marker: Option<PathBuf> field (polls ~/.maestro/last-pr-created); mascot_style: MascotStyle field hydrated in apply_config(); desktop_notifier: Arc<dyn DesktopNotifier>; notify_error_flash: Option<(String, Instant)>; with_desktop_notifier() test builder; tick_notify_error() drains take_last_error() and surfaces status-bar flash + LogLevel::Warn activity-log entry; OsascriptNotifier::new() wired in App::configure(); milestone_health_screen: Option<MilestoneHealthScreen> field + None init; attempted_pr_issue_numbers: HashSet<u64> in-process idempotency guard (prevents duplicate auto-PR attempts per run); rehydrates pending_issue_completions from persisted MaestroState on App::new(); mirrors pending_issue_completions back in sync_state(); pub mod auto_pr declaration; pub mod pushup_marker declaration; pub mod gate_retry declaration; pub mod completion_git declaration; git_ops: Box<dyn GitOps> field (production wires CliGitOps; tests inject MockGitOps via with_git_ops() test builder); shell_launcher: Arc<dyn ShellLauncher> field + with_shell_launcher() test setter; gate_runner field + with_gate_runner() test setter; call_log_state: CallLogState field for per-agent call-log viewer; `poll_interaction_auto_nav()` checks `lifecycle::poll_auto_nav` on the active interaction screen and emits `ScreenAction::Pop` when teardown is done  [Issue #12, #31-33, #35, #38, #40, #41, #43, #46-48, #52, #83, #84, #85, #86, #102, #104, #118, #123, #158, #159, #342, #437, #473, #487, #500, #514, #520, #545, #560, #562, #868, #741]
│   │   │   ├── types.rs                   # TuiMode enum (+ CompletionSummary, ContinuousPause, MilestoneHealth, AgentGraph, GateOutputViewer(Uuid), TeamWizard, CallLog(Uuid) variants) with breadcrumb_label() method; NavigationStack struct (push/pop/peek/clear/breadcrumbs, cap 32); TuiCommand enum (+ RunAdaptScaffold, FetchMilestoneHealthIssues, PatchMilestoneDescription, SendInteractionTurn); TuiDataEvent enum (+ AdaptScaffoldResult, MilestoneHealthIssuesFetched, MilestoneHealthPatched, InteractionTurnEvent, InteractionTurnComplete); SuggestionDataPayload; CompletionSummaryData; CompletionSessionLine (+ worktree_path: Option<PathBuf>); GateFailureInfo; has_failed_gates() and first_failed_gates_session() helpers on CompletionSummaryData; PendingIssueCompletion upgraded from pub(crate) to pub with Serialize/Deserialize/Clone/Debug derives (required for state persistence in #514); ScreenState gains team_wizard_screen field  [Issue #342, #371, #500, #514, #527, #560, #664, #868, #738]
...
│   │   │   ├── completion_pipeline.rs     # check_completions() config-driven gate evaluation with per-gate logging; persists worktree_path onto session at FailedGates transition; calls backup_wip_before_gates() BEFORE gate_runner.run_gates() then replaces the old commit_and_push() block with amend_or_commit_and_push()  [Issue #40, #104, #560, #562]
│   │   │   ├── completion_git.rs          # App::backup_wip_before_gates(): calls git_ops.backup_wip(branch) to create a WIP sentinel commit preserving all model work before gates run; App::amend_or_commit_and_push(): amends the WIP commit on success or leaves it in place on gate failure so the worktree is always recoverable; sanitize_log imported from src/util/sanitize.rs (#562 hoisted it out of auto_pr.rs)  [Issue #562]
│   │   │   ├── completion_summary.rs      # build_completion_summary(); populates gate_failures for FailedGates sessions (was NeedsReview only); transition_to_dashboard() calls navigate_to_root() to clear nav stack  [Issue #342, #560]
│   │   │   ├── context_overflow.rs        # Context overflow detection and fork triggering
│   │   │   ├── data_handler.rs            # handle_data_event(); data_tx/data_rx channel; SuggestionData, VersionCheckResult, UpgradeResult, AdaptScaffoldResult handlers; Milestones event routed to milestone_health_screen (in addition to milestone_screen); MilestoneHealthIssuesFetched and MilestoneHealthPatched event handlers; CiErrorLogFetched arm updates CiErrorReviewScreen fetch phase; TeamLaunchResult arm routes to wizard's apply_launch_result; InteractionTurnEvent/InteractionTurnComplete arms apply turn events to the live interaction screen + activity log, upsert session on completion  [Issue #371, #500, #695, #881, #738]
...
│   │   │   ├── gate_retry.rs              # App::retry_completion_gates(session_id): re-runs post-completion gates against a retained worktree; invoked by [g] in the failed-gates recovery modal  [Issue #560]
│   │   │   ├── helpers.rs                 # Shared App helper utilities
│   │   │   ├── auto_pr.rs                 # Auto-PR pipeline (#514, #520): pr_url() helper, sanitize_log import from src/util/sanitize.rs (function moved out in #562; re-exposes the #514 LOW-1 regression-mitigation at the new completion_git.rs call sites), App::run_auto_pr() — decides whether to create a PR; calls has_commits_ahead(worktree_path, branch, base_branch) between the AC4 PR-already-exists preflight and the issue-resolution step (zero-commit gate, #520); detects existing open PRs via list_prs_for_branch(), logs the PR URL on success, surfaces every gate-skip and error path with explicit activity-log entries, appends manual-recovery hint on general error; worktree_path: Option<PathBuf> arg threaded from PendingIssueCompletion  [Issue #514, #520, #562]
...
│   │   │   ├── session_spawners_resume_tests.rs  # Tests for spawn_resume_implement_session() and create_gate_fix_session(); split from session_spawners.rs to keep it under the 400-line cap  [Issue #695]
│   │   │   ├── ci_error_review.rs         # App::open_ci_error_review(); App::handle_ci_error_review_confirm(); FailingPrSnapshot; bridges CI monitor key event to CiErrorReviewScreen and spawn_ci_fix_session  [Issue #695]
│   │   │   ├── ci_error_review_tests.rs   # Tests for ci_error_review.rs; split into sibling file to stay under the 400-line cap  [Issue #695]
...
│   │   │   ├── tests.rs                   # App-level unit tests; 3 new defense-in-depth navigate_to tests for AgentGraph mode gate; 12 new tests for failed-gates summary and worktree_path plumbing  [Issue #527, #560]
...
│   │   ├── call_log/                      # Per-agent call-log viewer module (Issue #868)
│   │   │   ├── mod.rs                     # Public facade; event-payload formatters (render_event_payload with 10 KB per-field cap) and module re-exports
│   │   │   ├── state.rs                   # CallLogState: scrollable list state for call-log entries (capped at 500); scroll helpers
│   │   │   └── draw.rs                    # ratatui renderer: draws the CallLog overlay, entry rows, and formatted payload panel
...
│   │   ├── input_handler.rs               # Top-level key event dispatcher extracted from mod.rs; KeyAction enum (Consumed, Quit); handle_key() dispatches to overlay handlers, mode-specific input, global shortcuts, and screen dispatch in priority order; all Esc handlers use navigate_back_or_dashboard() via NavigationStack; 'c' key arm in handle_overview_keys() triggers copy_focused_response(); [e] key arm in CI monitor context calls open_ci_error_review(); handle_completion_summary() branches on has_failed_gates() — success path unchanged, failure path routes to handle_completion_summary_failed_gates(); handle_completion_summary_failed_gates() handles [s] open shell, [g] retry gates, [r] spawn resume session, [v] open GateOutputViewer, [q] close modal; handle_gate_output_viewer() handles paged scroll and [q]/[Esc] exit; [L] chord on Detail navigates to CallLog mode; handle_call_log() dispatcher (scroll + [q]/[Esc] exit); `switcher_target()` helper routes a live Interactive session from the switcher to its chat screen instead of Detail view (#950/#930); regression test updated for RunTeam round-trip; sibling test added for Err path  [Issue #342, #482, #560, #695, #868, #881, #950]
...
│   │   ├── ui.rs                          # ratatui rendering; budget display, TUI mode switching, notification banners, screen rendering branches; draw_upgrade_banner() renders upgrade notification states (available, downloading, installing, done, failed) as a top-of-screen banner with version info and [y]/[n] confirmation prompts; draw_gh_auth_warning() renders a persistent top-of-screen banner when gh CLI is not authenticated; CompletionSummary render branch and draw_completion_overlay() pub(crate): branches on has_failed_gates() — success path: centred overlay with PR links, outcome rows, and [i][r][l][q] bar; failure path: amber-bordered "Session Failed Gates" modal with worktree path, five keybindings ([s][g][r][v][q]), and gate failure lines; ContinuousPause render branch and continuous pause overlay; bottom bar split into info bar (agent count, cost, elapsed) and DOS-style F-key legend bar; draw_fkey_bar() renders amber-badged key names (F1–F10, Alt-X) with responsive width truncation; HelpBarContext struct drives context-aware keybinding dimming in the help bar; breadcrumb trail rendered in status bar from nav_stack.breadcrumbs() using TuiMode::breadcrumb_label(); should_show_dashboard_mascot_panel() / dashboard_mascot_panel_width() style-aware panel gate; passes MascotStyle through draw_mascot_block(), HomeScreen::set_mascot(), LandingScreen::set_mascot(); rendering arm for TuiMode::MilestoneHealth; rendering arm for TuiMode::GateOutputViewer dispatches to gate_output_viewer::draw(); draw() render-time gate reads views.agent_graph_enabled and dispatches to agent_graph::render::draw_agent_graph or falls back to panels view; AgentGraph dispatch arm threads crate::icon_mode::use_nerd_font(), spinner_tick, and &sessions into draw_agent_graph; dispatcher arm for TuiMode::CallLog(id) dispatches to call_log::draw::draw_call_log; Interaction arm calls InteractionScreen::set_view() with InteractionView built from the live Session each frame (#950)  [Phase 3, Issue #31-33, #83, #84, #85, #104, #118, #158, #218, #342, #473, #500, #527, #529, #560, #868, #950]
...
│   │   ├── token_dashboard.rs             # Token usage dashboard widget: per-session and aggregate token counts; TQ Ratio column removed (#346); `pub mod provider_rollup` declared; re-layout for per-provider rollup (`draw_provider_rollup`, `format_context_cell`, `format_quota_cell`, `with_thousands`, `truncate_provider`, `row_line`); `draw_token_dashboard_with_quota` testable inner + `NoQuotaSnapshots` zero-sized stub; stale `#850 will swap this` comment removed  [Issue #848, #849, #769]
...
│   │   ├── snapshot_tests/                # TUI snapshot tests using insta (173+ tests, 21 views)  [Issue #16, #490, #526, #527, #528, #539, #543, #560, #568, #569, #664, #714, #715, #716, #785, #741]
│   │   │   ├── mod.rs                     # Module declarations for snapshot test submodules; mod agent_graph, mod agent_graph_dispatcher, mod agent_graph_keybinding_hint, mod agent_personalities, mod activity_log_dispatch, mod completion_overlay, mod team_wizard, mod schema_tab, mod settings_project_parity, and 12 additional settings_*_parity modules wired (including settings_budget_parity added in #785, settings_teams_parity added in #803, settings_teams_warnings_parity added in #909); `make_session_with_agent` helper added; `mod token_dashboard_provider_rollup` declared; `mod call_log` declared (5 snapshots: empty, single entry, multi-entry, scrolled, formatted payload); `mod interaction_terminator` declared (4 snapshots: idle-success, streaming-deferred, teardown-failure, userquit-before-terminator); `mod interaction` extended with settled-banner tests (settled_completed, settled_with_pr, settled_failed_gates) in #950  [Issue #526, #527, #528, #539, #543, #560, #664, #714, #715, #716, #785, #803, #849, #868, #909, #741, #950]
...
│   │   │   ├── agent_graph.rs             # 14 snapshot tests for agent graph: renders_at_80x24, renders_at_100x30, renders_at_120x40, single_agent_with_files_as_graph, falls_back_when_single_agent_has_no_files (single-agent fallback narrowed in #543: graph now renders whenever there is at least one agent + one file edge); agent_graph_running_node_braille_spinner_at_tick_0, agent_graph_running_node_braille_spinner_at_tick_5, agent_graph_completed_flash_tick4_bold_reversed, agent_graph_edge_pulse_tooluse_tick0, agent_graph_edge_pulse_tooluse_tick5, agent_graph_ascii_fallback_spinner_running_node; render helper signature updated to accept tick + nerd-font flag; file_labels_grow_outward and file_label_truncates_with_ellipsis added (four_cardinal_files_session helper); file_node_marker_and_label_visually_connected added (1-agent+3-files, pins marker-removed label-at-edge-endpoint fix)  [Issue #526, #529, #543, #568, #569]
...
│   │   │   ├── completion_overlay.rs      # 80×24 snapshot tests for both completion modal variants: snapshot_completion_overlay_success_modal_80x24 (success path — [i][r][l][q] bar unchanged) and snapshot_completion_overlay_failed_gates_modal_80x24 (failure path — amber border, [s][g][r][v][q] bar); uses make_failed_gates_session() fixture from mod.rs and draw_completion_overlay() (pub(crate))  [Issue #560]
│   │   │   ├── team_wizard.rs             # ~33 snapshot tests for TeamWizardScreen across all flows × sizes; heads tested at 60×20 / 80×24 / 120×40; terminal states (Success, Error, Confirm) tested at 80×24; covers Compose (Source→Save), Launch (TeamPicker→Confirm), Manage (list, delete-confirm, edit-redirect)  [Issue #664]
│   │   │   ├── schema_tab.rs              # Snapshot test for the schema-driven renderer: schema_tab_synth_table_renders_80x24 locks the visual contract for all six FieldKind leaf variants and NestedTable flattening  [Issue #714]
│   │   │   ├── settings_project_parity.rs # Parity snapshot tests for Project tab: verifies schema renderer output matches the hand-coded renderer at 80×24  [Issue #715]
│   │   │   ├── settings_review_parity.rs  # Parity snapshot tests for Review tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_turboquant_parity.rs  # Parity snapshot tests for TurboQuant tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_notifications_parity.rs  # Parity snapshot tests for Notifications tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_github_parity.rs  # Parity snapshot tests for GitHub tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_layout_parity.rs  # Parity snapshot tests for Layout tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_theme_parity.rs   # Parity snapshot tests for Theme tab: verifies schema renderer output at 80×24  [Issue #716]
│   │   │   ├── settings_gates_parity.rs   # Parity snapshot tests for Gates tab: verifies schema renderer output at 80×24 (gates.ci_auto_fix reshaped as NestedTable child)  [Issue #716]
│   │   │   ├── settings_sessions_parity.rs  # Parity snapshot tests for Sessions tab: verifies schema renderer output at 80×24 (sessions.hollow_retry, sessions.context_overflow, sessions.conflict reshaped as NestedTable children)  [Issue #716]
...
│   │   │   ├── call_log.rs                 # 5 snapshot tests for the CallLog view: empty state, single entry, multi-entry list, scrolled position, formatted payload panel  [Issue #868]
│   │   │   ├── interaction_terminator.rs  # 4 snapshot tests for the terminator UI flow: idle-success, streaming-deferred, teardown-failure, userquit-before-terminator  [Issue #741]
│   │   │   └── snapshots/                 # Committed insta snapshot files (.snap files); includes caveman_row renders (default, error, explicit_false, explicit_true, focused_explicit_true); copy_keybinding_hint_enabled and copy_keybinding_hint_disabled; agent_graph renders at 80x24, 100x30, 120x40, and single_agent_card; agent_graph_dispatcher toggle_on and toggle_off baselines; agent_graph_keybinding_hint snapshots for flag-on/flag-off overview hints and agent_graph mode hints; 11 agent_personalities snapshots added; 12 activity_log_dispatch snapshots added (chip rendering per role × mode, guard cases, multi-dispatch); 2 completion_overlay snapshots added (success and failed-gates modal variants); file_labels_grow_outward and file_label_truncates_with_ellipsis added (per-side anchor and ellipsis truncation); file_node_marker_and_label_visually_connected added (marker-removed label-at-endpoint regression baseline); all agent_graph_* baselines refreshed; ~33 team_wizard snapshots added (all flows × 60×20 / 80×24 / 120×40 heads + 80×24 terminal states); schema_tab_synth_table_renders_80x24 snapshot added for schema-driven renderer; project_tab_flag_off_renders_80x24 snapshot for Project tab parity; 9 additional *_tab_flag_off_renders_80x24 snapshots added for Review, TurboQuant, Notifications, GitHub, Layout, Theme, Gates, Sessions, Advanced tabs; budget_tab_renders_80x24 and budget_tab_renders_120x40 snapshots added for Budget tab float-precision parity; 7 token_dashboard_provider_rollup snapshots added (per-provider rollup cost/context/quota table); teams_tab_empty_renders_80x24 and teams_tab_one_entry_renders_80x24 snapshots added for Teams tab parity; teams_tab_one_entry_with_role_overrides_renders_80x24 snapshot added for role_overrides (field-count bumped from 4 to 5; snapshot updated in #901 to reflect fully editable DynamicMap)  [Issue #490, #482, #526, #527, #528, #539, #543, #560, #568, #569, #664, #714, #715, #716, #785, #803, #849, #872, #901]
│   │   ├── screen_dispatch.rs             # ScreenDispatch: routes key events and render calls to the active screen; constructor receives FeatureFlags for settings screen injection; always injects prompt history when constructing PromptInputScreen; ScreenAction::Push delegates to navigate_to(), ScreenAction::Pop delegates to navigate_back(); Scaffolding case in StartAdaptPipeline dispatch; reads app.config_path directly for settings save (removed relative-path probe at TuiMode::Settings); tracing::warn! when config_path is absent; MilestoneHealth Push/Pop arms added; drains milestone_health_screen pending command channel after each input; `open_interaction_session` (create + re-entry skip-dialog); interaction LaunchSession branch; SendInteractionTurn → command; QuitInteraction handler (terminate + keep worktree)  [Issue #146, #232, #342, #371, #437, #500, #738]
│   │   └── screens/                       # Interactive screen components  [Issue #31-33]
│   │       ├── mod.rs                     # Screen types: ScreenAction enum (+ RefreshSuggestions, SendInteractionTurn, QuitInteraction variants), SessionConfig; re-exports HomeScreen, IssueBrowserScreen, MilestoneScreen; pub mod wizard_fields (added #447); pub mod milestone_health (added #500); pub mod gate_output_viewer (added #560); pub mod interaction (added #738); wizard_paste removed  [Issue #31-33, #86, #447, #500, #560, #738]
...
│   │       ├── ci_error_review.rs         # CiErrorReviewScreen: popup overlay over Overview; FetchPhase enum (Loading, Ready, Failed); CiErrorReviewState; shows failed CI check names, log excerpt, and local gate command; [Enter] launches fix session, [Esc] dismisses; [e] keybind added to CI monitor footer  [Issue #695]
│   │       ├── gate_output_viewer.rs      # GateOutputViewer: full-screen paged renderer for gate stderr; scrollable via ↑/↓; receives gate stderr text from the retained session's gate_results; [q]/[Esc] returns to completion overlay  [Issue #560]
...
│   │       ├── interaction/               # Interactive iteration session screen  [Issue #738]
│   │       │   ├── mod.rs                 # InteractionScreen state: issue_number, produce_pr, worktree_path, branch, worktree_root, queued_terminator, terminated, view fields; `for_managed` ctor; `set_view(InteractionView)` per-frame injection seam; screen owns no transcript (#950); keymap-driven `handle_input`; KeymapProvider impl in keymap.rs  [Issue #738, #741, #950]
│   │       │   ├── keymap.rs              # Pure `classify(state, produce_pr, key, mods) -> InteractionIntent` + `pushup_prompt(n)`; all key routing logic isolated here; KeymapProvider impl (moved from mod.rs in #741)  [Issue #738, #741]
│   │       │   ├── keymap_tests.rs        # Unit tests for `classify()` covering all intents × state combinations  [Issue #738]
│   │       │   ├── lifecycle.rs           # Terminator UI flow: `WorktreeTeardownPort` + `Clock` seams; `on_terminator_signaled`, `drain_queued_terminator`, `fire_terminator`, `poll_auto_nav`; `#[cfg(test)] MockTeardown` + `FakeClock`  [Issue #741]
│   │       │   ├── terminator_tests.rs    # Unit tests for the terminator UI lifecycle: idle-success, streaming-deferred, teardown-failure, userquit-before-terminator paths  [Issue #741]
│   │       │   ├── turn.rs                # Turn lifecycle: `begin_turn`, `handle_quit_modal`; `apply_turn_event` and `push_turn` removed in #950 (pipeline writes turns to `Session.turns`); deferred `drain_queued_terminator` call on `TurnFinished`  [Issue #738, #741, #950]
│   │       │   ├── input.rs               # Render helpers: locked input pane, greyed Ctrl+P keybar, quit-confirm modal, terminated banner  [Issue #738]
│   │       │   ├── history.rs             # Transcript history rendering helpers  [Issue #738]
│   │       │   └── tests.rs               # Additional screen-level unit tests  [Issue #738]
...
│   │       │   ├── types.rs               # Landing screen type definitions; LandingMenuItem::push("Milestone Review", 'h', TuiMode::MilestoneHealth) added; `[t] Teams` menu item added emitting PushTeamWizard  [Issue #500, #664]
...
│   │       ├── team_wizard/               # Team Wizard TUI screen: Compose / Launch / Manage flows  [Issue #664]
│   │       │   ├── mod.rs                 # TeamWizardScreen: owns step machines for all three flows + session caches (resolved_teams, preflight, issue_metas); apply_resolved_teams(), apply_health_check(), apply_issue_metas() setters for dispatcher wiring (future)
│   │       │   ├── types.rs               # ComposeStep, LaunchStep, ManageStep enums; ComposePayload, LaunchPayload, ManageState, PreflightCache, AgentHealth, TeamLaunchInput, TeamWizardMode types
│   │       │   ├── draw.rs                # ratatui rendering dispatch; routes to compose/launch/manage draw functions; all external strings wrapped with sanitize_for_terminal (ANSI-escape injection hardening)
...
│   │           ├── tabs/                  # Per-tab module files for schema-driven settings tabs  [Issue #792, #803]
...
│   │           │   └── teams.rs           # `SettingsTab::Teams` (index 9): renders `[teams]` via `FlattenedMap` + `TEAMS_TABLE` schema; delegates to DynamicMapWidget with bindings encoded as StringList of `role=agent` pairs  [Issue #803]
...
│   │           │   ├── teams_bindings.rs  # Pure adapter: `encode_bindings()` flattens `[teams.<id>]` top-level scalar keys into StringList of `role=agent` items; `decode_bindings()` reverses the round-trip; `RESERVED_KEYS` const is the only schema-level coupling to `TeamConfig`'s `#[serde(flatten)]` field — update here when bindings semantics change  [Issue #803]
...
│   │           │   ├── integration.rs     # End-to-end SettingsScreen save/load integration tests
│   │           │   ├── keybinds_save_caveman.rs  # Keybinding and caveman-mode save tests
│   │           │   └── schema_flag.rs     # Flag-wiring tests: verifies flag routing and field counts across settings tabs; Flag::SchemaDrivenSettings deleted in #716 (legacy flag-off/flag-on split removed; all tabs unconditionally use schema renderer)  [Issue #714, #715, #716]
...
│   │   └── 003-minimax-quota-persistence.md # ADR 003: MiniMax quota persistence — fs2 vs fslock vs std::fs::File::lock, ~/.maestro path, sliding-window vs counter, schema_version=1 + deny_unknown_fields; tracking issue #775
...
│   │   ├── minimax-quota.json             # Schema for ~/.maestro/minimax-quota.json — schema_version:1 + requests array of RFC 3339 timestamps; additionalProperties:false  [Issue #775]
...
│   ├── insight/                           # Generated artifact directory for maestro insight scan output  [Issue #1002]
│   │   └── scan.json                      # Versioned scan artifact: repo_stats + modules (P1); features + coverage (P2); design_system — palette/icons/mascot populated (P3); architecture populated by later phases; schema governed by src/insight/schema.rs  [Issue #1002, #1003, #1004]
...
│   ├── teams/                             # Team orchestration preset documentation (v0.26.0+)  [Issue #665]
│   │   ├── README.md                      # Preset overview, three-tier resolution model, five built-ins table, CLI surface, headless launch semantics, and state migration guide; User Guide section links to cookbook  [Issue #675]
│   │   ├── cookbook/                      # Hands-on team configuration recipes  [Issue #675]
│   │   │   ├── 01-when-to-use-a-team.md   # Decision guide: when a team preset adds value vs single-agent run
│   │   │   ├── 02-extending-a-builtin.md  # How to extend a built-in preset with `extends`
│   │   │   ├── 03-composing-from-scratch.md  # Full walkthrough: composing a custom team TOML from scratch
│   │   │   ├── 04-recipes.md              # Copy-paste recipes for common team configurations
│   │   │   └── 05-troubleshooting.md      # Troubleshooting guide for team configuration and launch issues
...
│   │   │   ├── teams_add.toml.before      # Before-fixture for teams round-trip: validates that adding a `[teams.<id>]` entry with bindings encodes and decodes correctly via the StringList adapter  [Issue #803]
...
│   │       ├── cheap-coder.toml           # Minimal coder preset with reduced reviewer depth
│   │       ├── fanout-reviewers.toml      # Fan-out preset distributing review across multiple agents
│   │       ├── inbox-triager.toml         # Idea-inbox bulk triage preset fixture
│   │       ├── repo-policy-coder.toml     # Coder preset enforcing repo-level policy gates
│   │       ├── ship-it.toml               # Expedited pipeline preset skipping non-critical gates
│   │       └── strict-reviewer.toml       # Reviewer preset with strict acceptance criteria
...
└── maestro.toml                           # Runtime configuration; [sessions.context_overflow] section; guardrail_prompt option (commented); [sessions.completion_gates] with fmt, clippy, test defaults; [sessions.hollow_retry] section (policy, work_max_retries, consultation_max_retries); [project] gains optional language/languages/build_command/test_command/run_command fields (written by `maestro init --reset`); [views] section with agent_graph_enabled = true (default-on since v0.25.1 #710; older files without this key are migrated automatically on first startup)  [Issue #12, #40, #43, #275, #505, #525, #710]
```
...
| `docs/adr/003-minimax-quota-persistence.md` | ADR 003 — MiniMax quota persistence; fs2 lock choice, ~/.maestro path, sliding window, schema_version=1; tracking issue #775 |
| `docs/api-contracts/minimax-quota.json` | JSON Schema for `~/.maestro/minimax-quota.json`; schema_version:1 + RFC 3339 timestamp array; additionalProperties:false (Issue #775) |
...
| `src/flags/mod.rs` | `Flag` enum with 7 variants: `ContinuousMode`, `AutoFork`, `CiAutoFix`, `ReviewCouncil`, `ModelRouting`, `ContextOverflow`, `TurboQuant`; `FlagSource` enum (`Default`, `Config`, `Cli`); `serde` derive; `default_enabled()`, `description()`, `name()`, `all()` helpers |
...
| `src/git.rs` | `GitOps` trait, `CliGitOps`: commit+push; `list_remote_branches()`; `has_commits_ahead(branch, base)` — zero-commit gate consumed by auto-PR pipeline (#520); `WIP_SUBJECT_PREFIX`/`SUFFIX` constants; `backup_wip(branch)`, `amend_clean_and_push(branch)`, `head_is_wip_backup(branch)` trait methods + `CliGitOps` impls; `--` separator before branch positional in all push calls; leading-`-` refusal in `commit_and_push`; `MockGitOps` extracted to `git_mock.rs`; tests extracted to `git_tests.rs` (Phase 3, Issue #159, #514, #520, #562) |
...
| `src/templates/manifest.rs` | `Manifest::load(path)` — deserializes `.maestro/templates/manifest.toml`; `#[serde(deny_unknown_fields)]` enforces forward-compatibility; `ManifestSubagent { slug, purpose }` struct added; `Manifest::subagents()` accessor returns `&[ManifestSubagent]` (Issues #701, #728) |
| `src/templates/rendered_cache.rs` | `RenderedTemplateStore` trait + `DiskRenderedTemplateStore` impl + `FakeRenderedStore` test double; disk impl reads pre-rendered template bodies from the provider's cache dir; installed on `SessionPool` and consulted by `try_promote` to inject template body for HTTP-generic providers when `origin == DirectUser` and `active_command.is_some()` (Issue #707) |
| `src/templates/provider_rules/mod.rs` | `TemplateProviderRules` trait; `NullRules` fail-closed stub; `null_rules()` helper returns `&'static NullRules`; re-exports `ClaudeRules`, `codex_rules()`, and `http_generic_rules()`; `read_sandboxed` + `read_skill_body` shared helpers consolidate the three formerly per-file sandbox-aware readers; Unix-gated `symlink_escape_is_rejected_by_starts_with_check` test (Issues #701, #703, #704, #705, #727) |
| `src/templates/provider_rules/claude.rs` | `ClaudeRules` — concrete `TemplateProviderRules` for Claude Code; `include()` delegates to `read_sandboxed`; 8 unit tests (Issues #703, #727) |
| `src/templates/provider_rules/codex.rs` | `CodexRules` — concrete `TemplateProviderRules` for Codex; `target_dir()=None`; `invoke_subagent` inlines as `## Sub-task: {name}`; `hook_gate` emits provider-neutral `bash .maestro/hooks/<script>`; `include()` delegates to `read_sandboxed`; `skill_link` delegates to `read_skill_body`; `subagent_list` delegates to `subagent_list::load_subagent_list_markdown()` (Issues #704, #727, #728) |
| `src/templates/provider_rules/http_generic.rs` | `HttpGenericRules` — shared `TemplateProviderRules` for HTTP-transport providers (Ollama, MiniMax) and Qwen; `target_dir()=None`; `hook_gate` renders orchestrator instruction text instead of a shell command; `include()` delegates to `read_sandboxed`; `skill_link` delegates to `read_skill_body`; `http_generic_rules()` singleton; 12 unit tests (Issues #705, #727, #728) |
| `src/templates/provider_rules/subagent_list.rs` | Shared `{{SUBAGENT_LIST}}` renderer; `render_subagent_list_default(&[ManifestSubagent])` — pure fn, pipeline-order rows, no trailing newline; `load_subagent_list_markdown()` — parses manifest and delegates; 7 unit tests including byte-identical check against the live manifest (Issue #728) |
| `tests/subagent_manifest_drift.rs` | Drift guard between `.claude/agents/subagent-*.md` on disk and `[[subagents]]` slugs in `manifest.toml`; `check_drift()` pure comparator; 5 unit tests + 1 live-repo integration test; fix guidance points to `manifest.toml` `[[subagents]]` block (Issue #728) |
...
| `src/integration_tests/templates_runtime.rs` | Integration tests for HTTP-provider runtime template injection via `SessionPool::try_promote`; uses `FakeRenderedStore` to verify body injection when `origin == DirectUser` and `active_command.is_some()`; also covers no-op paths (origin mismatch, store absent, `target_dir` set) (Issue #707) |
...
| `src/config/transport_warning.rs` | Pre-cutoff startup warning for the headless claude transport (fires from 2026-05-15; `Clock` trait + `MockClock`; `MAESTRO_SILENCE_TRANSPORT_WARN=1` read by the caller) (Issue #750) |
| `src/work/activity.rs` | `InteractionActivity` enum — pinned `[INTERACTION]`/`[TEARDOWN]` dashboard line formats + `emit_tracing()`; `ActivitySeverity`; consumed via `ActivityLog::emit_interaction` and `screens::interaction::activity_action` (Issue #742) |
| `docs/guides/claude-transport.md` | Headless vs interactive transport user guide + 2026-06-15 cutoff migration (Issue #752) |
| `docs/guides/interaction-sessions.md` | Interaction sessions user guide: launch combos, keymap, lifecycle, troubleshooting (Issue #743) |
| `docs/spikes/2026-05-claude-interactive-transport.md` | Spike #747 feasibility report (GREEN): PTY drive + transcript tailing + env-scrub validation; prototype at `docs/spikes/prototypes/portable-pty-prototype.rs` |
| `testdata/claude-transcript/` | Canned 3-turn transcript fixture + stub `claude` REPL script for interactive-transport tests (Issue #749) |
| `src/agent_provider/claude/mod.rs` | `ClaudeProvider` impl; `ClaudeTransport` enum + `from_config_value()`; `InteractiveDriver::PortablePty`; transport dispatcher (text one-shots stay headless under Interactive); `interactive_slots` parked-children store; `with_transport()` constructor; `template_rules()` override returns `&'static ClaudeRules` (Issues #703, #748, #749, #750, #751) |
| `src/agent_provider/claude/headless.rs` | Headless subprocess logic relocated from the old `claude.rs` flat file; `--resume <id>` from `AgentRequest.resume_session_id`; session-id capture into `AgentRunResult.session_id` (Issues #748, #751) |
| `src/agent_provider/claude/interactive.rs` | Interactive (PTY) transport turn loop: `InteractiveBackend`/`InteractiveChild` traits, `run_session_turn` (park/reuse children keyed by session id, `--resume` re-attach with history skip, Ctrl-C cancel keeps child), transcript tail in `spawn_blocking` → bounded mpsc(64), `ANTHROPIC_*` env scrub (Issues #749, #751) |
| `src/agent_provider/claude/pty_backend.rs` | `portable-pty` production backend: openpty 132x40, env scrub/set, continuous drain thread, child reaped in Drop (Issue #749) |
| `src/agent_provider/claude/transcript_parser.rs` | Claude Code session-transcript JSONL → `StreamEvent` mapping; `turn_duration` → `Completed`; benign metadata skipped; contract at `docs/api-contracts/claude-transcript.json` (Issue #749) |
| `src/agent_provider/claude/interactive_tests.rs` | Mock-backend unit tests + real-PTY stub-claude e2e (`testdata/claude-transcript/stub-claude.sh`) for the interactive transport (Issues #749, #751) |
...
| `src/agent_provider/codex/parser_tests.rs` | Split test module for `codex/parser.rs` (no behavior change; extracted to stay under 400-LOC cap) (Issue #772) |
| `src/agent_provider/codex/pricing.rs` | Per-model Codex pricing — gpt-5-codex, gpt-5, o3; cached-input tier falls back to regular input rate when no cached rate is published; `compute_cost(model, usage)` free fn (Issue #772) |
| `src/agent_provider/codex/tests.rs` | Codex provider integration tests |
| `src/agent_provider/qwen.rs` | `QwenProvider` impl; `template_rules()` override returns `http_generic_rules()` (Issue #705) |
| `src/agent_provider/qwen_parser.rs` | Qwen stream parser |
| `src/agent_provider/qwen_tests.rs` | Qwen parser tests |
| `src/agent_provider/ollama/mod.rs` | `OllamaProvider` impl; emits `ContextUpdate` from `[agents.<id>].num_ctx` config field; `template_rules()` override returns `http_generic_rules()` (Issues #705, #774) |
| `src/agent_provider/ollama/http.rs` | Ollama HTTP transport |
| `src/agent_provider/ollama/tests.rs` | Ollama provider tests |
| `src/agent_provider/ollama/types.rs` | Ollama-specific types |
| `src/agent_provider/openai_compat/mod.rs` | OpenAI-compatible module re-exports |
| `src/agent_provider/openai_compat/sse.rs` | Shared OpenAI-compatible SSE parser; now extracts `prompt_tokens` / `completion_tokens` / `prompt_tokens_details.cached_tokens` from the final SSE frame (Issue #773) |
| `src/agent_provider/openai_compat/sse_tests.rs` | Split test module for `openai_compat/sse.rs` (extracted to stay under 400-LOC cap) (Issue #773) |
| `src/agent_provider/opencode.rs` | OpenCode provider facade |
| `src/agent_provider/opencode/parser.rs` | OpenCode stream parser; falls back to `opencode/pricing.rs` when telemetry reports `cost: 0` (Issue #773) |
| `src/agent_provider/opencode/pricing.rs` | Per-model OpenCode pricing fallback; applied when `step_finish.cost == 0`; unknown models return `0.0` (Issue #773) |
| `src/agent_provider/opencode/cost_tests.rs` | Split test module for OpenCode cost/pricing logic (Issue #773) |
| `src/agent_provider/opencode/tests.rs` | OpenCode provider integration tests |
...
| `src/agent_provider/parser.rs` | Shared agent stream parser helpers |
| `src/agent_provider/test_fakes.rs` | Shared test fakes for agent provider tests |
...
| `src/provider/github/ci.rs` | `CiChecker` (`check_pr_status`, `fetch_failure_log`); `CiStatus`; `CiPollAction`; `PendingPrCheck`; `build_ci_fix_prompt`; `truncate_log`; `parse_ci_json`; `decide_ci_action`; `local_gate_for_check()` maps a check name to a local gate command string (Issues #41, #695) |
...
| `src/session/interaction.rs` | `InteractionSession`, `TurnRole`, `CloseReason`, `TurnRecord`, `TurnState` types; `queued_terminator` field; `signal_terminator()` / `fire_terminator()` state-machine methods; `settled_banner()` helper; `Session::interactive_banner()`; `InteractionState` removed in #950 (Issue #734, #739, #950) |
...
| `src/tui/mod.rs` | Event loop; `handle_screen_action()`; command processing; `launch_session_from_config()`; `FetchSuggestionData` async handler for GitHub ready/failed counts and milestone progress; spawns async version check on startup via `check_for_update()` — result delivered as `VersionCheckResult` data event; key handlers for upgrade confirmation banner (`[y]` confirm / `[n]` decline); `CompletionSummary` key-intercept branch with `[i]` issue browser, `[r]` new prompt, `[l]` activity log view, `[Enter]`/`[Esc]` dashboard; `ContinuousPause` key-intercept overlay: `[s]` skip, `[r]` retry, `[q]` quit continuous loop; exit path respects `once_mode`; `PromptInputScreen` always constructed with injected history for correct Up/Down recall; `pub mod theme`; `RunAdaptScaffold` command dispatch; `pub(crate) mod agent_graph` wired; background fetch task for CI error log (`TuiCommand::LaunchCiErrorReview` → spawns async fetch and delivers `TuiDataEvent::CiErrorLogFetched`); `pub mod call_log` registered (Issue #868); command-pump arm for `TuiCommand::RunTeam` spawns `RealTeamLauncher::run_team()` and posts `TuiDataEvent::TeamLaunchResult` (Issue #881); command-pump arm for `TuiCommand::SendInteractionTurn` calls `send_turn` on a session clone via `ClaudeCliSpawner`, streams `TurnEvent`s back over `data_tx`, writes mutated session back (Issue #738); event-loop tick calls `poll_interaction_auto_nav()` and emits `ScreenAction::Pop` when teardown completes (Issue #741) (Issues #31-33, #35, #38, #46-48, #83, #84, #85, #118, #232, #371, #526, #695, #868, #881, #738, #741) |
| `src/tui/team_runner_glue.rs` | `RealTeamLauncher`: production `TeamLauncher` bridge wrapping `data_tx + ProviderConfig`; used by the TUI command-pump to hand off to `run_team()` (Issue #881) |
...
| `src/tui/agent_graph/model.rs` | `GraphNode`, `GraphEdge`, `NodeId`, `NodeKind`, `Positioned` data types; `build_graph()` signature updated to `&[&Session]`; `NodeId::Team(Uuid)` variant added; `NodeKind::Team { primitive, member_count }` variant added; `build_graph_with_teams()` entry point added (Issues #526, #527, #664) |
| `src/tui/agent_graph/layout.rs` | `Viewport` struct; `Layout` trait; `ConcentricLayout` impl (inner agent ring, barycenter-sorted outer file ring); `cell_aspect()` returns `0.5` for aspect-ratio correction applied to x coordinates; match arms extended for `NodeId::Team` and `NodeKind::Team` (Issues #526, #664) |
...
| `src/tui/agent_graph/label_placement.rs` | `LabelPlacement`: computes the largest angular gap among outbound edges from an agent and places the `#NNN` issue-number chip at that gap's midpoint (north when no edges); anchor is shifted by `(1 − cos θ) / 2 × label_width` cells to keep east/west labels clear of their adjacent edge bundle; `place_file_label()` per-side anchor logic (right-half grows rightward, left-half grows leftward, `FILE_LABEL_DEAD_BAND` center band stays centered); `truncate_with_ellipsis()` for labels that overshoot available outward span; `FILE_LABEL_BORDER_MARGIN_CELLS` constant (Issue #567, #568) |
| `src/tui/agent_graph/render.rs` | ratatui `Canvas` renderer; draws edges as `Line` shapes, agent nodes as `Rectangle` + label, file nodes as label-only (no marker rectangle, label anchored at edge endpoint per #569); sprite/abbrev rendering via `personalities` module; single-agent fallback card; 80×24 min-size guard; agent chip placement delegated to `label_placement::LabelPlacement`; file-arm labels placed via `label_placement::place_file_label`; `NodeKind::Team` render arm added (container box with member_count label) (Issues #526, #539, #567, #568, #569, #664) |
| `src/tui/agent_graph/render_label_placement_tests.rs` | Regression snapshot tests for label placement: `agent_label_does_not_overlap_south_edge` and `agent_label_does_not_overlap_south_edge_nerd_font` pin the corrected chip position for the one-agent-two-south-files layout (Issue #567) |
| `src/tui/app/` | App state module split into focused sub-files; `App` struct with `nav_stack: NavigationStack` field (replaces `confirm_exit_return_mode`); `navigate_to()`, `navigate_back()`, `navigate_back_or_dashboard()`, `navigate_to_root()` navigation methods; `theme: Theme`; `gh_auth_ok: bool`; `upgrade_state: UpgradeState`; `pending_prs: Vec<PendingPr>`; `config_path: Option<PathBuf>` propagated from `LoadedConfig` for settings save; `desktop_notifier: Arc<dyn DesktopNotifier>`; `notify_error_flash: Option<(String, Instant)>`; `tick_notify_error()` per-frame error drain; `pub minimax_quota: Option<Arc<MinimaxQuota>>` field + `App::with_minimax_quota` builder for wiring live quota into TUI render sites; `enabled_agent_ids()` and `build_agent_provider_map()` relocated to `src/commands/agent_provider_map.rs` in Issue #1000 — call sites repointed to `crate::commands::agent_provider_map::*` (Issues #12, #31-33, #35, #38, #40, #41, #43, #46-48, #52, #83, #84, #85, #118, #158, #342, #437, #487, #769, #1000) |
| `src/tui/app/types.rs` | `TuiMode` enum (+ `AgentGraph`, `GateOutputViewer(Uuid)`, `CiErrorReview`, `TeamWizard` variants) with `breadcrumb_label()` for human-readable mode names; `NavigationStack` struct — push/pop/peek/clear/breadcrumbs with a cap of 32 entries; `TuiCommand` (+ `RunAdaptScaffold`, `LaunchCiErrorReview`, `RunTeam { scheduler, app_default_agent }`, `SendInteractionTurn`), `TuiDataEvent` (+ `AdaptScaffoldResult`, `CiErrorLogFetched`, `TeamLaunchResult(Result<(), String>)`, `InteractionTurnEvent`, `InteractionTurnComplete`), `SuggestionDataPayload`, `CompletionSummaryData`, `CompletionSessionLine` (+ `worktree_path: Option<PathBuf>`), `GateFailureInfo`; `has_failed_gates()` and `first_failed_gates_session()` helpers on `CompletionSummaryData`; `ScreenState` gains `team_wizard_screen: Option<TeamWizardScreen>` field (Issues #342, #371, #527, #560, #664, #695, #881, #738) |
| `src/tui/app/completion_summary.rs` | `build_completion_summary()`; populates `gate_failures` for `FailedGates` sessions (previously populated for `NeedsReview` only); `transition_to_dashboard()` now calls `navigate_to_root()` to fully clear the nav stack on dashboard return (Issues #342, #560) |
...
| `src/tui/app/gate_retry.rs` | `App::retry_completion_gates(session_id)`: re-runs the post-completion gate suite against the retained worktree; invoked by `[g]` in the failed-gates recovery modal (Issue #560) |
| `src/tui/app/ci_error_review.rs` | `App::open_ci_error_review()`: builds a `FailingPrSnapshot` from the currently-selected PR and pushes `TuiMode::CiErrorReview`; `App::handle_ci_error_review_confirm()`: confirms the popup and calls `spawn_ci_fix_session`; `FailingPrSnapshot` struct (Issue #695) |
| `src/tui/app/ci_error_review_tests.rs` | Unit tests for `ci_error_review.rs`; split into sibling file to stay under the 400-line cap (Issue #695) |
| `src/tui/app/session_spawners_resume_tests.rs` | Tests for `spawn_resume_implement_session()` and `create_gate_fix_session()`; split from `session_spawners.rs` to stay under the 400-line cap (Issue #695) |
| `src/tui/app/completion_git.rs` | `App::backup_wip_before_gates()`: creates a WIP sentinel commit via `git_ops.backup_wip()` before gates run, guaranteeing model edits survive gate failure, crash, or manual worktree removal; `App::amend_or_commit_and_push()`: amends the WIP commit on gate success, leaving it as a safety net on failure; `sanitize_log` imported from `src/util/sanitize.rs` (function hoisted out of `auto_pr.rs` in #562) (Issue #562) |
| `src/git_mock.rs` | `MockGitOps`: test double extracted from `git.rs` to keep both files under the 400-LOC cap; configurable stubs for all `GitOps` trait methods including `backup_wip`, `amend_clean_and_push`, `head_is_wip_backup` (Issue #562) |
| `src/git_tests.rs` | Unit tests for `git.rs` extracted to sibling file (attached via `#[cfg(test)] #[path]`); covers flag-injection refusal, WIP sentinel subject line, amend happy path, `head_is_wip_backup` predicate, and `--` separator in push argv (Issue #562) |
| `src/integration_tests/wip_backup.rs` | Pipeline-level regression tests using real git in a tempdir; covers the full backup→gate→amend-or-retain sequence and validates that model work is never lost on gate failure (Issue #562) |
| `src/tui/shell_launcher.rs` | `ShellLauncher` trait + `OsShellLauncher` impl (`$SHELL` spawned via `std::process::Command`); `FakeShellLauncher` test double; injected into `App` via `with_shell_launcher()`; invoked by `[s]` in the failed-gates recovery modal (Issue #560) |
| `src/tui/screens/gate_output_viewer.rs` | `GateOutputViewer`: full-screen paged view of gate stderr; keyboard-scrollable; dispatched by `[v]` in the failed-gates recovery modal via `TuiMode::GateOutputViewer(Uuid)`; `[q]`/`[Esc]` pops back to the completion overlay (Issue #560) |
| `src/tui/screens/ci_error_review.rs` | `CiErrorReviewScreen`: popup overlay shown in Overview when `[e]` is pressed on the CI monitor; `FetchPhase` enum (`Loading`, `Ready { log_excerpt }`, `Failed { reason }`); `CiErrorReviewState`; `CiFixConfig`; `sanitize_for_terminal()`; renders failed check names, log excerpt, and local gate command; `[Enter]` confirms and launches fix session, `[Esc]` dismisses (Issue #695) |
| `src/tui/snapshot_tests/completion_overlay.rs` | 2 insta snapshot tests for the completion overlay at 80×24: `snapshot_completion_overlay_success_modal_80x24` (success path — hint bar `[i][r][l][q]`) and `snapshot_completion_overlay_failed_gates_modal_80x24` (failed-gates path — amber border, hint bar `[s][g][r][v][q]`); uses `make_failed_gates_session()` fixture and `draw_completion_overlay()` (pub(crate)) (Issue #560) |
...
| `src/tui/ui.rs` | `draw_upgrade_banner()`: top-of-screen banner that renders all `UpgradeState` variants; `draw_gh_auth_warning()`: persistent top-of-screen banner shown when gh CLI is not authenticated, blocks gh-dependent actions until resolved; `draw_completion_overlay()` (pub(crate)): branches on `has_failed_gates()` — success path: centred overlay with PR links (underlined), per-session outcome rows, and `[i][r][l][q]` bar; failed-gates path: amber-bordered "Session Failed Gates" modal with worktree path and `[s][g][r][v][q]` recovery bar; `ContinuousPause` render branch with pause overlay and status bar indicator; render arm for `TuiMode::GateOutputViewer(id)` dispatches to `gate_output_viewer::draw()`; render arm for `TuiMode::CiErrorReview` dispatches to `ci_error_review::draw()`; render arm and screen-resolution arm for `TuiMode::TeamWizard` dispatches to `team_wizard::draw()`; `HelpBarContext` struct drives context-aware keybinding dimming in the help bar; breadcrumb trail rendered in status bar from `nav_stack.breadcrumbs()` using `TuiMode::breadcrumb_label()`; `should_show_dashboard_mascot_panel()` / `dashboard_mascot_panel_width()` style-aware panel gate; passes `MascotStyle` through `draw_mascot_block()`, `HomeScreen::set_mascot()`, `LandingScreen::set_mascot()`; uses `keybinding_hints_spans()` for hint bar; ticks and renders clipboard toast overlay; `draw()` reads `views.agent_graph_enabled` and dispatches to `agent_graph::render::draw_agent_graph` or falls back to panels view; `draw_token_dashboard_with_app_quota` helper unifies both TokenDashboard render sites to thread `app.minimax_quota` into the widget (Issues #83, #84, #85, #118, #158, #218, #342, #473, #482, #527, #560, #664, #695, #769) |
| `src/tui/screens/` | Interactive TUI screen components (Issues #31-33) |
| `src/tui/screens/mod.rs` | `ScreenAction` enum, `SessionConfig`; re-exports all screen types including `PromptInputScreen`; adds `pub mod wizard_fields`; adds `pub mod gate_output_viewer`; adds `pub mod ci_error_review`; adds `pub mod team_wizard`; adds `pub mod interaction`; `ScreenAction::PushTeamWizard { mode, preselect }`, `ScreenAction::SendInteractionTurn`, `ScreenAction::QuitInteraction` variants added; `LaunchCiFix` and `CiFixConfig` re-exported; removes `wizard_paste` (Issues #31-33, #86, #447, #560, #664, #695, #738) |
...
| `src/tui/screens/milestone.rs` | `MilestoneScreen`: milestone list with progress gauge and run-all action; `t` keybinding emits `ScreenAction::PushTeamWizard` with milestone slice preselect (Issues #33, #664) |
...
| `src/tui/screens/interaction/` | Interactive iteration session screen: per-turn prompt/response loop against a running `claude --resume` session (Issue #738) |
| `src/tui/screens/interaction/mod.rs` | `InteractionScreen` state (issue_number, produce_pr, worktree_path, branch, worktree_root, queued_terminator, terminated, view fields); `for_managed` ctor; `set_view(InteractionView)` per-frame injection from app; screen owns no transcript (#950); `KeymapProvider` impl in `keymap.rs` (Issue #738, #741, #950) |
| `src/tui/screens/interaction/keymap.rs` | Pure `classify(state, produce_pr, key, mods) -> InteractionIntent` function + `pushup_prompt(n)` builder; all key routing isolated here for testability; `KeymapProvider` impl (moved from `mod.rs` in #741) (Issue #738, #741) |
| `src/tui/screens/interaction/keymap_tests.rs` | Unit tests for `classify()` covering all `InteractionIntent` variants × state combinations (Issue #738) |
| `src/tui/screens/interaction/lifecycle.rs` | Terminator UI flow: `WorktreeTeardownPort` + `Clock` seams; `on_terminator_signaled`, `drain_queued_terminator`, `fire_terminator`, `poll_auto_nav`; `#[cfg(test)] MockTeardown` + `FakeClock` (Issue #741) |
| `src/tui/screens/interaction/terminator_tests.rs` | Unit tests for the terminator UI lifecycle: idle-success, streaming-deferred, teardown-failure, userquit-before-terminator paths (Issue #741) |
| `src/tui/screens/interaction/turn.rs` | Turn lifecycle: `begin_turn`, `handle_quit_modal`, `apply_turn_event`; deferred `drain_queued_terminator` call on `TurnFinished` (Issue #738, #741) |
| `src/tui/screens/interaction/input.rs` | Render helpers: locked input pane while turn is running, greyed Ctrl+P keybar, quit-confirm modal overlay, terminated banner (Issue #738) |
| `src/tui/screens/interaction/history.rs` | Transcript history rendering helpers (Issue #738) |
...
| `src/tui/screens/issue_browser/mod.rs` | `IssueBrowserScreen`: multi-select list with label/milestone filters; `set_issues()` delivers async data; `reapply_filters()` honours active filters when new data arrives; `t` keybinding emits `ScreenAction::PushTeamWizard` with issue preselect (Issues #32, #46, #117, #664) |
...
| `src/tui/screens/team_wizard/` | Team Wizard TUI screen: Compose / Launch / Manage flows for orchestration teams (Issue #664) |
| `src/tui/screens/team_wizard/mod.rs` | `TeamWizardScreen`: step-machine orchestrator for all three flows; owns `ComposePayload`, `LaunchPayload`, `ManageState`, `PreflightCache`; entry via `new()` or `with_entry(mode, preselect)`; async-data setters `apply_resolved_teams()`, `apply_health_check()`, `apply_issue_metas()` are integration points for future dispatcher wiring (Issue #664) |
| `src/tui/screens/team_wizard/types.rs` | `ComposeStep`, `LaunchStep`, `ManageStep` enums; `ComposePayload`, `LaunchPayload`, `ManageState`, `PreflightCache`, `AgentHealth`, `TeamLaunchInput`, `TeamWizardMode`, `ComposeSource` types (Issue #664) |
| `src/tui/screens/team_wizard/draw.rs` | ratatui rendering dispatcher; all external strings (issue/milestone titles, team names, agent IDs, failure_reason, last_error) wrapped with `sanitize_for_terminal` to prevent ANSI-escape injection (Issue #664) |
...
| `src/tui/screen_dispatch.rs` | `ScreenDispatch`: routes key events and render calls to the active screen; constructor accepts `FeatureFlags` to supply the settings screen; always injects prompt history when constructing `PromptInputScreen`; `ScreenAction::Push` delegates to `navigate_to()`, `ScreenAction::Pop` delegates to `navigate_back()`; `Scaffolding` case wired in `StartAdaptPipeline` dispatch; reads `app.config_path` directly for settings save (removed relative-path probe); `tracing::warn!` when `config_path` is absent; `CiErrorReview` dispatch arm added; `PushTeamWizard` arm constructs `TeamWizardScreen::with_entry()`; Push/Pop arms for `TuiMode::TeamWizard`; `ScreenAction::LaunchTeam` arm rewritten to push `TuiCommand::RunTeam`; `open_interaction_session` (create + re-entry skip-dialog, logs `[INTERACTION] resumed #N`); interaction `LaunchSession` branch; `SendInteractionTurn` → `TuiCommand`; `QuitInteraction` handler (terminate + keep worktree) (Issues #146, #232, #342, #371, #437, #664, #695, #881, #738) |
...
| `src/tui/snapshot_tests/agent_graph.rs` | 16 insta snapshot tests for agent graph: `agent_graph_renders_at_80x24/100x30/120x40` (Issue #526); `agent_graph_renders_single_agent_with_files_as_graph` + `agent_graph_falls_back_when_single_agent_has_no_files` (Issue #543 narrowed the fallback to only the no-edges case); plus 6 animation/spinner tests (Issue #529); `file_labels_grow_outward` and `file_label_truncates_with_ellipsis` added via `four_cardinal_files_session` helper (Issue #568); `agent_sprite_contiguous_at_80x24`, `agent_sprite_contiguous_at_120x40`, `agent_sprite_contiguous_at_200x60` added for viewport-derived sprite sizing (Issue #576) |
...
| `src/tui/snapshot_tests/schema_tab_modals.rs` | 2 insta snapshot tests for schema-tab Add/Remove modals at 80×24: `add_entry_modal_renders_80x24` and `remove_confirm_modal_renders_80x24`; verifies both modals use the themed double-bordered block (Issue #808) |
| `src/tui/snapshot_tests/` | TUI snapshot test suite; 169+ tests across 20 views using `insta`; run with `cargo test tui::snapshot_tests`; update with `INSTA_UPDATE=always cargo test` or `cargo insta review` (Issues #16, #490, #526, #527, #539, #542, #543, #560, #568, #714, #715, #785, #808) |
| `src/tui/snapshot_tests/settings_project_parity.rs` | 8 parity snapshot tests for the Project settings tab: flag-off path renders via hand-coded renderer, flag-on path renders via `from_schema(&PROJECT_TABLE, config)` + `sync_to_config`; both paths snapshot-locked at 80×24 to detect renderer divergence (Issue #715) |
| `src/tui/snapshot_tests/settings_budget_parity.rs` | 8 parity tests for Budget tab after schema-renderer migration; verifies float display correctness (`per_session_usd` = 5.5, `total_usd` = 12.5) at 80×24 and 120×40; 2 snapshot baselines + 6 assertion-only tests (Issue #785) |
...
| `src/tui/snapshot_tests/interaction_terminator.rs` | 4 snapshot tests for the terminator UI flow: idle-success, streaming-deferred, teardown-failure, userquit-before-terminator (Issue #741) |

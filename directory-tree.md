...
│   │   │   ├── roundtrip_role_overrides.rs  # 3 round-trip tests for role_overrides: empty map omits bare header, populated map survives save/reload, passthrough preserves existing keys through EntryState  [Issue #872]
...
│   │   │   ├── settings_teams_parity.rs   # Parity snapshot tests for Teams tab: verifies schema renderer output (empty state, one-entry with bindings round-trip, one-entry with role_overrides passthrough) at 80×24; role_overrides renders as TextInput placeholder in v0.29.x — full editor deferred to follow-up  [Issue #803, #872]
...
│   │           │   ├── teams_tab.rs       # Integration tests for Teams tab: covers empty state, one-entry round-trip (bindings encode/decode), and validate_extends error surfacing on unknown parent  [Issue #803]

...
│   ├── tui/
│   │   ├── mod.rs                         # Event loop; keybindings; handle_screen_action() rewritten; command processing loop; launch_session_from_config(); FetchSuggestionData async handler spawns background GitHub fetch for ready/failed counts and milestone progress; CompletionSummary key-intercept branch dismisses overlay and returns to Dashboard; exit path checks once_mode — exits immediately when true, otherwise shows CompletionSummary overlay; pub mod theme  [Phase 3, Issue #31-33, #46-48, #35, #38, #83]
│   │   ├── app.rs                         # App state; TuiMode (+ CompletionSummary variant); TuiCommand enum (FetchIssues, FetchMilestones, FetchSuggestionData, LaunchSession, LaunchSessions); TuiDataEvent enum (Issues, Milestones, Issue, SuggestionData); SuggestionDataPayload; CompletionSummaryData struct; CompletionSessionLine struct; once_mode: bool field; completion_summary: Option<CompletionSummaryData> field; build_completion_summary(); return_to_dashboard(); handle_data_event(); data_tx/data_rx channel; pending_commands; check_completions() uses config-driven gates with per-gate activity logging; poll_ci_status() with CI auto-fix loop; spawn_ci_fix_session(); on_issue_session_completed() skips PR creation for CI-fix sessions; issue launch path uses PromptBuilder::build_issue_prompt(); theme: Theme field built from config in configure()  [Issue #12, #31-33, #35, #38, #40, #41, #43, #46-48, #52, #83]
...
│   │   ├── ui.rs                          # ratatui rendering; budget display, TUI mode switching, notification banners, screen rendering branches; CompletionSummary render branch and draw_completion_summary() centred overlay with per-session outcome rows  [Phase 3, Issue #31-33, #83]
...
| `src/tui/mod.rs` | Event loop; `handle_screen_action()`; command processing; `launch_session_from_config()`; `FetchSuggestionData` async handler for GitHub ready/failed counts and milestone progress; `CompletionSummary` key-intercept branch; exit path respects `once_mode`; `pub mod theme` (Issues #31-33, #35, #38, #46-48, #83) |
| `src/tui/app.rs` | `App` struct with `theme: Theme` field built in `configure()`; `TuiMode` (+ `CompletionSummary` variant); `CompletionSummaryData` and `CompletionSessionLine` structs; `once_mode: bool`; `completion_summary` field; `build_completion_summary()`; `return_to_dashboard()`; `TuiCommand` (adds `FetchSuggestionData`); `TuiDataEvent` (adds `SuggestionData`); `SuggestionDataPayload`; `handle_data_event()`; `data_tx`/`data_rx` channel; `check_completions()` config-driven gates with per-gate logging; `poll_ci_status()` with CI auto-fix loop; `spawn_ci_fix_session()`; `on_issue_session_completed()` skips PR creation for CI-fix sessions; issue launch uses `PromptBuilder::build_issue_prompt()` (Issues #12, #31-33, #35, #38, #40, #41, #43, #46-48, #52, #83) |

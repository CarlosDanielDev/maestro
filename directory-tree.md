...
│   │   │   ├── label_placement.rs         # LabelPlacement: computes the largest angular gap between all outbound edges from an agent node and places the #NNN chip at the midpoint of that gap (north fallback when no edges); anchor shifted by (1 − cos θ) / 2 × label_width to avoid the edge bundle on east/west placements  [Issue #567]
│   │   │   ├── render.rs                  # ratatui Canvas renderer; draws edges as Lines, nodes as Rectangles + labels; sprite/abbrev rendering via personalities module; status_modifier() helper; single-agent fallback card; 80×24 min-size guard; signature extended with use_nerd_font: bool, tick: usize, sessions: &[&Session]; Running node labels carry braille/ASCII spinner glyph; edge pulse on current_activity; Completed/Errored flash via parity-modulated BOLD/BOLD|REVERSED; tests extracted to render_tests.rs sibling; label placement delegated to label_placement module  [Issue #526, #527, #529, #539, #567]
...
│   │   ├── snapshot_tests/                # TUI snapshot tests using insta (107 tests, 16 views)  [Issue #16, #490, #526, #527, #528, #539, #543, #560]
...
| `src/tui/agent_graph/label_placement.rs` | `LabelPlacement`: computes the largest angular gap among outbound edges from an agent and places the `#NNN` issue-number chip at that gap's midpoint (north when no edges); anchor is shifted by `(1 − cos θ) / 2 × label_width` cells to keep east/west labels clear of their adjacent edge bundle (Issue #567) |
| `src/tui/agent_graph/render.rs` | ratatui `Canvas` renderer; draws edges as `Line` shapes, nodes as `Rectangle` + label; sprite/abbrev rendering via `personalities` module; single-agent fallback card; 80×24 min-size guard; label placement for `#NNN` chip delegated to `label_placement` module (Issues #526, #539, #567) |
...
| `src/tui/snapshot_tests/` | TUI snapshot test suite; 107 tests across 16 views using `insta`; run with `cargo test tui::snapshot_tests`; update with `INSTA_UPDATE=always cargo test` or `cargo insta review` (Issues #16, #490, #526, #527, #539, #542, #543, #560) |

...
│   │   ├── snapshot_tests/                # TUI snapshot tests using insta (110 tests, 16 views)  [Issue #16, #490, #526, #527, #528, #539, #543, #560, #568, #569]
│   │   │   ├── mod.rs                     # Module declarations for snapshot test submodules; mod agent_graph, mod agent_graph_dispatcher, mod agent_graph_keybinding_hint, mod agent_personalities, mod activity_log_dispatch, and mod completion_overlay wired; make_failed_gates_session() fixture helper  [Issue #526, #527, #528, #539, #543, #560]
...
| `src/tui/ui.rs` | `draw_upgrade_banner()`: top-of-screen banner that renders all `UpgradeState` variants; `draw_gh_auth_warning()`: persistent top-of-screen banner shown when gh CLI is not authenticated, blocks gh-dependent actions until resolved; `draw_completion_overlay()` (pub(crate)): branches on `has_failed_gates()` — success path: centred overlay with PR links (underlined), per-session outcome rows, and `[i][r][l][q]` bar; failed-gates path: amber-bordered "Session Failed Gates" modal with worktree path and `[s][g][r][v][q]` recovery bar; `ContinuousPause` render branch with pause overlay and status bar indicator; render arm for `TuiMode::GateOutputViewer(id)` dispatches to `gate_output_viewer::draw()`; render arm for `TuiMode::CiErrorReview` dispatches to `ci_error_review::draw()`; `HelpBarContext` struct drives context-aware keybinding dimming in the help bar; breadcrumb trail rendered in status bar from `nav_stack.breadcrumbs()` using `TuiMode::breadcrumb_label()`; `should_show_dashboard_mascot_panel()` / `dashboard_mascot_panel_width()` style-aware panel gate; passes `MascotStyle` through `draw_mascot_block()`, `HomeScreen::set_mascot()`, `LandingScreen::set_mascot()`; uses `keybinding_hints_spans()` for hint bar; ticks and renders clipboard toast overlay; `draw()` reads `views.agent_graph_enabled` and dispatches to `agent_graph::render::draw_agent_graph` or falls back to panels view (Issues #83, #84, #85, #118, #158, #218, #342, #473, #482, #527, #560, #695) |
| `src/tui/screens/` | Interactive TUI screen components (Issues #31-33) |
| `src/tui/screens/mod.rs` | `ScreenAction` enum, `SessionConfig`; re-exports all screen types including `PromptInputScreen`; adds `pub mod wizard_fields`; adds `pub mod gate_output_viewer`; adds `pub mod ci_error_review`; `LaunchCiFix` and `CiFixConfig` re-exported; removes `wizard_paste` (sanitizer moved into `TextAreaField::insert_sanitized`) (Issues #31-33, #86, #447, #560, #695) |

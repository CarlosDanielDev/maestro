...
│   │   ├── mod.rs                         # Module facade; `scan(root) -> Scan` orchestrator + `run_cli(root) -> Result<()>` CLI entry point; populates repo_stats and modules (P1); features/design_system/architecture/coverage sections present but empty (later phases)  [Issue #1002]
...
│   │   ├── modules.rs                     # `analyze_source(path, src) -> Module` — syn-based extractor; collects public API names, doc comments, and use-edge imports from Rust source files  [Issue #1002]

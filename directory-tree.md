...
│   │       ├── mod.rs                     # Screen types: ScreenAction enum (+ RefreshSuggestions variant), SessionConfig; re-exports HomeScreen, IssueBrowserScreen, MilestoneScreen; pub mod wizard_fields (added #447); pub mod milestone_health (added #500); pub mod gate_output_viewer (added #560); wizard_paste removed  [Issue #31-33, #86, #447, #500, #560]
...
| `src/tui/screens/mod.rs` | `ScreenAction` enum, `SessionConfig`; re-exports all screen types including `PromptInputScreen`; adds `pub mod wizard_fields`; adds `pub mod gate_output_viewer`; adds `pub mod ci_error_review`; adds `pub mod team_wizard`; `ScreenAction::PushTeamWizard { mode, preselect }` variant added; `LaunchCiFix` and `CiFixConfig` re-exported; removes `wizard_paste` (sanitizer moved into `TextAreaField::insert_sanitized`) (Issues #31-33, #86, #447, #560, #664, #695) |
...
| `src/tui/screen_dispatch.rs` | `ScreenDispatch`: routes key events and render calls to the active screen; constructor accepts `FeatureFlags` to supply the settings screen; always injects prompt history when constructing `PromptInputScreen`; `ScreenAction::Push` delegates to `navigate_to()`, `ScreenAction::Pop` delegates to `navigate_back()`; `Scaffolding` case wired in `StartAdaptPipeline` dispatch; reads `app.config_path` directly for settings save (removed relative-path probe); `tracing::warn!` when `config_path` is absent; `CiErrorReview` dispatch arm added — forwards key events to `CiErrorReviewScreen` and emits `ScreenAction` on confirm/dismiss; `PushTeamWizard` arm constructs `TeamWizardScreen::with_entry()`; Push/Pop arms and screen-resolution arm for `TuiMode::TeamWizard` added; `ScreenAction::LaunchTeam` arm rewritten to push `TuiCommand::RunTeam` (replaces the former `tracing::warn!("LaunchTeam fanned out via LaunchSession path …")` placeholder) (Issues #146, #232, #342, #371, #437, #664, #695, #881) |

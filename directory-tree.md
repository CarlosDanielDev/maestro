...
│   │       │   ├── mod.rs                 # InteractionScreen state: issue_number, produce_pr, worktree_path, branch, worktree_root, queued_terminator, terminated_at, teardown, clock fields; `for_session` ctor; keymap-driven `handle_input`; KeymapProvider impl moved to keymap.rs (400-line cap)  [Issue #738, #741]
...
│   │       │   ├── turn.rs                # Turn lifecycle: `begin_turn`, `handle_quit_modal`, `apply_turn_event`; deferred `drain_queued_terminator` call on `TurnFinished`  [Issue #738, #741]
...
| `src/tui/screens/interaction/mod.rs` | `InteractionScreen` state (issue_number, produce_pr, worktree_path, branch, worktree_root, queued_terminator, terminated_at, teardown, clock fields); `for_session` ctor; keymap-driven `handle_input`; `KeymapProvider` impl moved to `keymap.rs` to respect 400-line cap (Issue #738, #741) |

...
│   │   ├── mod.rs                         # Module facade; `scan(root) -> Scan` orchestrator + `run_cli(root) -> Result<()>` CLI entry point; populates repo_stats and modules (P1); collects features via `collect_features` (4 surfaces: cli, tui_mode, slash_command, subagent) and maps modules + coverage (P2)  [Issue #1002, #1003]
...
│   │   └── scan.json                      # Versioned scan artifact: repo_stats + modules (P1); features/design_system/architecture/coverage populated by later phases; schema governed by src/insight/schema.rs  [Issue #1002]

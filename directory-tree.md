...
│   │   ├── mod.rs                         # Flag enum (7 variants, including SchemaDrivenSettings default-off); FlagSource enum (Default, Config, Cli); serde serialization; default_enabled(), description(), name(), all() helpers  [Issue #714]
...
│   │   ├── snapshot_tests/                # TUI snapshot tests using insta (151+ tests, 19 views)  [Issue #16, #490, #526, #527, #528, #539, #543, #560, #568, #569, #664, #714, #715]
│   │   │   ├── mod.rs                     # Module declarations for snapshot test submodules; mod agent_graph, mod agent_graph_dispatcher, mod agent_graph_keybinding_hint, mod agent_personalities, mod activity_log_dispatch, mod completion_overlay, mod team_wizard, mod schema_tab, and mod settings_project_parity wired; make_failed_gates_session() fixture helper  [Issue #526, #527, #528, #539, #543, #560, #664, #714, #715]
...
│   │           ├── schema_tab/            # Schema-driven settings renderer — builds SettingsField entries from a TableSchema; wired behind the schema_driven_settings feature flag (default off); Project tab is the first production consumer via from_schema(&PROJECT_TABLE, config) + sync_to_config  [Issue #714, #715]
...
│   │           │   └── schema_flag.rs     # Flag-wiring tests: schema_driven_settings is registered, default-off; Project tab is first migrated tab — flag-on routes Project through schema renderer, flag-off uses hand-coded path; field counts verified across both paths  [Issue #714, #715]

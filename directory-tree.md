...
│   │   │   ├── session_spawners.rs        # spawn_gate_fix_session(); build_gate_fix_prompt(); launch_session_from_config(); spawn_resume_implement_session(line) spawns a /implement #N --continue session against the retained worktree  [Issue #560]
...
| `src/tui/app/types.rs` | `TuiMode` enum (+ `AgentGraph`, `GateOutputViewer(Uuid)` variants) with `breadcrumb_label()` for human-readable mode names; `NavigationStack` struct — push/pop/peek/clear/breadcrumbs with a cap of 32 entries; `TuiCommand` (+ `RunAdaptScaffold`), `TuiDataEvent` (+ `AdaptScaffoldResult`), `SuggestionDataPayload`, `CompletionSummaryData`, `CompletionSessionLine` (+ `worktree_path: Option<PathBuf>`), `GateFailureInfo`; `has_failed_gates()` and `first_failed_gates_session()` helpers on `CompletionSummaryData` (Issues #342, #371, #527, #560) |

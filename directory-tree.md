...
│   │       │   ├── mod.rs                 # InteractionScreen state: issue_number, produce_pr, worktree_path, state machine, quit_modal_open, stream counters; `for_session` ctor; keymap-driven `handle_input`; expanded keybindings  [Issue #738]
│   │       │   ├── keymap.rs              # Pure `classify(state, produce_pr, key, mods) -> InteractionIntent` + `pushup_prompt(n)`; all key routing logic isolated here  [Issue #738]
│   │       │   ├── keymap_tests.rs        # Unit tests for `classify()` covering all intents × state combinations  [Issue #738]
│   │       │   ├── turn.rs                # Turn lifecycle: `begin_turn`, `handle_quit_modal`, `apply_turn_event`; folds `TurnEvent` stream into the screen transcript  [Issue #738]
...
| `src/tui/screens/interaction/mod.rs` | `InteractionScreen` state (issue_number, produce_pr, worktree_path, state machine, quit_modal_open, stream counters); `for_session` ctor; keymap-driven `handle_input`; expanded keybindings; keymap: Enter=send, Shift+Enter=newline, Ctrl+P=pushup (greyed when produce_pr=false), Ctrl+L=clear, Esc=back (session keeps running), Ctrl+Q=quit-confirm (Issue #738) |
| `src/tui/screens/interaction/keymap.rs` | Pure `classify(state, produce_pr, key, mods) -> InteractionIntent` function + `pushup_prompt(n)` builder; all key routing isolated here for testability (Issue #738) |
...
| `src/tui/screens/interaction/turn.rs` | Turn lifecycle: `begin_turn`, `handle_quit_modal`, `apply_turn_event`; folds `TurnEvent` stream into the screen transcript (Issue #738) |
